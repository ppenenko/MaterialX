...
uniform vec3 conductor_brdf1_artistic_ior_reflectivity = vec3(0.944000, 0.776000, 0.373000);
uniform vec3 conductor_brdf1_artistic_ior_edge_color = vec3(0.998000, 0.981000, 0.751000);
uniform vec3 Emission_EDF_color = vec3(0.200000, 1.000000, 0.200000);
uniform vec2 roughness_dual1_roughness = vec2(0.200000, 0.400000);
uniform float sheen_brdf1_weight = 1.000000;
uniform vec3 sheen_brdf1_color = vec3(1.000000, 1.000000, 1.000000);
uniform float sheen_brdf1_roughness = 0.300000;
uniform int sheen_brdf1_mode = 0;
uniform float subsurface_brdf1_weight = 1.000000;
uniform vec3 subsurface_brdf1_color = vec3(0.180000, 0.180000, 0.180000);
uniform vec3 subsurface_brdf1_radius = vec3(1.000000, 1.000000, 1.000000);
uniform float subsurface_brdf1_anisotropy = 0.000000;
uniform float conductor_brdf1_weight = 1.000000;
uniform vec2 conductor_brdf1_roughness = vec2(0.000000, 0.000000);
uniform bool conductor_brdf1_retroreflective = false;
uniform float conductor_brdf1_thinfilm_thickness = 0.000000;
uniform float conductor_brdf1_thinfilm_ior = 1.500000;
uniform int conductor_brdf1_distribution = 0;
uniform float mix_edf1_mix = 0.500000;
uniform float SchlickBRDF_layer_top_weight = 1.000000;
uniform vec3 SchlickBRDF_layer_top_color0 = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 SchlickBRDF_layer_top_color82 = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 SchlickBRDF_layer_top_color90 = vec3(1.000000, 1.000000, 1.000000);
uniform float SchlickBRDF_layer_top_exponent = 5.000000;
uniform bool SchlickBRDF_layer_top_retroreflective = false;
uniform float SchlickBRDF_layer_top_thinfilm_thickness = 0.000000;
uniform float SchlickBRDF_layer_top_thinfilm_ior = 1.500000;
uniform int SchlickBRDF_layer_top_distribution = 0;
uniform int SchlickBRDF_layer_top_scatter_mode = 0;
uniform float mix_bsdf1_mix = 0.500000;
uniform float surface1_opacity = 1.000000;
uniform bool surface1_thin_walled = false;
uniform float surface2_opacity = 1.000000;
uniform bool surface2_thin_walled = false;
uniform float mix_surface1_mix = 0.500000;
...
void main()
{
    ...
    vec3 conductor_brdf1_artistic_ior_ior = vec3(0.0);
    vec3 conductor_brdf1_artistic_ior_extinction = vec3(0.0);
    mx_artistic_ior(conductor_brdf1_artistic_ior_reflectivity, conductor_brdf1_artistic_ior_edge_color, conductor_brdf1_artistic_ior_ior, conductor_brdf1_artistic_ior_extinction);
    vec2 roughness_dual1_out = vec2(0.0);
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF conductor_brdf1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_conductor_bsdf(closureData, conductor_brdf1_weight, conductor_brdf1_artistic_ior_ior, conductor_brdf1_artistic_ior_extinction, conductor_brdf1_roughness, conductor_brdf1_retroreflective, conductor_brdf1_thinfilm_thickness, conductor_brdf1_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, conductor_brdf1_distribution, conductor_brdf1_out);

            surface1_out.color += occlusion * conductor_brdf1_out.response;
        }
        ...
        // Add surface emission
        {
            ...
            mx_uniform_edf(closureData, Emission_EDF_color, Emission_EDF_out);
            EDF mix_edf1_out = EDF(0.0);
            mx_mix_edf(closureData, Emission_EDF_out, EDF(0.0), mix_edf1_mix, mix_edf1_out);
            surface1_out.color += mix_edf1_out;
        }

        // Calculate the BSDF transmission for viewing direction
        BSDF conductor_brdf1_out = BSDF(vec3(0.0),vec3(1.0));
        surface1_out.color += conductor_brdf1_out.response;
    ...
    surfaceshader surface2_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        vec3 N = normalize(vd.normalWorld);
        vec3 V = normalize(u_viewPosition - vd.positionWorld);
        vec3 P = vd.positionWorld;
        ...
        float surfaceOpacity = surface2_opacity;
        ...
        {
            ClosureData closureData = makeClosureData(CLOSURE_TYPE_INDIRECT, L, V, N, P, occlusion);
            BSDF SchlickBRDF_layer_top_out = BSDF(vec3(0.0),vec3(1.0));
            mx_generalized_schlick_bsdf_broken(closureData, SchlickBRDF_layer_top_weight, SchlickBRDF_layer_top_color0, SchlickBRDF_layer_top_color82, SchlickBRDF_layer_top_color90, SchlickBRDF_layer_top_exponent, roughness_dual1_out, SchlickBRDF_layer_top_retroreflective, SchlickBRDF_layer_top_thinfilm_thickness, SchlickBRDF_layer_top_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, SchlickBRDF_layer_top_distribution, SchlickBRDF_layer_top_scatter_mode, SchlickBRDF_layer_top_out);
            BSDF sheen_brdf1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_sheen_bsdf(closureData, sheen_brdf1_weight, sheen_brdf1_color, sheen_brdf1_roughness, geomprop_Nworld_out1, sheen_brdf1_mode, sheen_brdf1_out);
            BSDF subsurface_brdf1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_subsurface_bsdf(closureData, subsurface_brdf1_weight, subsurface_brdf1_color, subsurface_brdf1_radius, subsurface_brdf1_anisotropy, geomprop_Nworld_out1, subsurface_brdf1_out);
            BSDF mix_bsdf1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_mix_bsdf(closureData, sheen_brdf1_out, subsurface_brdf1_out, mix_bsdf1_mix, mix_bsdf1_out);
            BSDF SchlickBRDF_out = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_bsdf(closureData, SchlickBRDF_layer_top_out, mix_bsdf1_out, SchlickBRDF_out);

            surface2_out.color += occlusion * SchlickBRDF_out.response;
        }
        ...
        // Add surface emission
        {
            ...
            EDF Emission_EDF_out = EDF(0.0);
            mx_uniform_edf(closureData, Emission_EDF_color, Emission_EDF_out);
            surface2_out.color += Emission_EDF_out;
        }
        ...
        BSDF SchlickBRDF_out = BSDF(vec3(0.0),vec3(1.0));
        mx_layer_bsdf(closureData, SchlickBRDF_layer_top_out, mix_bsdf1_out, SchlickBRDF_out);
        surface2_out.color += SchlickBRDF_out.response;

        // Compute and apply surface opacity
        {
            surface2_out.color *= surfaceOpacity;
            surface2_out.transparency = mix(vec3(1.000000, 1.000000, 1.000000), surface2_out.transparency, surfaceOpacity);
        }
    }

    surfaceshader mix_surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    mx_mix_surfaceshader(surface1_out, surface2_out, mix_surface1_mix, mix_surface1_out);
    out11 = vec4(mix_surface1_out.color, 1.0);
}
