...
uniform float conductor_brdf_1_weight = 1.000000;
uniform vec3 conductor_brdf_1_ior = vec3(0.180000, 0.420000, 1.370000);
uniform vec3 conductor_brdf_1_extinction = vec3(3.420000, 2.350000, 1.770000);
uniform vec2 conductor_brdf_1_roughness = vec2(0.000000, 0.000000);
uniform bool conductor_brdf_1_retroreflective = false;
uniform float conductor_brdf_1_thinfilm_thickness = 550.000000;
uniform float conductor_brdf_1_thinfilm_ior = 1.500000;
uniform int conductor_brdf_1_distribution = 0;
...
void main()
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF conductor_brdf_1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_conductor_bsdf(closureData, conductor_brdf_1_weight, conductor_brdf_1_ior, conductor_brdf_1_extinction, conductor_brdf_1_roughness, conductor_brdf_1_retroreflective, conductor_brdf_1_thinfilm_thickness, conductor_brdf_1_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, conductor_brdf_1_distribution, conductor_brdf_1_out);

            surface1_out.color += occlusion * conductor_brdf_1_out.response;
        }

        // Calculate the BSDF transmission for viewing direction
        BSDF conductor_brdf_1_out = BSDF(vec3(0.0),vec3(1.0));
        surface1_out.color += conductor_brdf_1_out.response;
