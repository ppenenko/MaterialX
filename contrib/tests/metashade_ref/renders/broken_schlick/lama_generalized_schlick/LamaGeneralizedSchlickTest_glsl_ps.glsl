...
uniform vec3 LamaGeneralizedSchlick_reflectionTint_cm_in = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 LamaGeneralizedSchlick_transmissionTint_cm_in = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 LamaGeneralizedSchlick_reflectivity_cm_in = vec3(0.040000, 0.040000, 0.040000);
uniform vec3 LamaGeneralizedSchlick_reflectivity90_cm_in = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 LamaGeneralizedSchlick_absorptionColor_cm_in = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 LamaGeneralizedSchlick_scatterColor_cm_in = vec3(0.000000, 0.000000, 0.000000);
uniform int LamaGeneralizedSchlick_fresnelMode = 0;
uniform vec3 LamaGeneralizedSchlick_IOR = vec3(1.500000, 1.500000, 1.500000);
uniform float LamaGeneralizedSchlick_reflectivityProfile = 0.200000;
uniform float LamaGeneralizedSchlick_roughness = 0.100000;
uniform float LamaGeneralizedSchlick_anisotropy = -1.000000;
uniform float LamaGeneralizedSchlick_anisotropyRotation = 0.000000;
uniform float LamaGeneralizedSchlick_absorptionRadius = 1.000000;
uniform float LamaGeneralizedSchlick_scatterAnisotropy = 0.000000;
uniform float LamaGeneralizedSchlickSurface_opacity = 1.000000;
uniform bool LamaGeneralizedSchlickSurface_thin_walled = false;
...
void IMPL_lama_generalized_schlick(ClosureData closureData, vec3 reflectionTint, vec3 transmissionTint, int fresnelMode, vec3 IOR, vec3 reflectivity, vec3 reflectivity90, float reflectivityProfile, float roughness, vec3 normal, float anisotropy, vec3 anisotropyDirection, float anisotropyRotation, vec3 absorptionColor, float absorptionRadius, vec3 scatterColor, float scatterAnisotropy, out BSDF out1)
{
    vec3 convert_ior_out = vec3(0.0);
    NG_convert_vector3_color3(IOR, convert_ior_out);
    const float exponent_in1_tmp = 1.000000;
    float exponent_out = exponent_in1_tmp / reflectivityProfile;
    const float anisotropy_switch_value2_tmp = 0.000000;
    float anisotropy_switch_out = (anisotropy > anisotropy_switch_value2_tmp) ? anisotropy : roughness;
    ...
    const float ior_sub_one_in2_tmp = 1.000000;
    vec3 ior_sub_one_out = convert_ior_out - ior_sub_one_in2_tmp;
    const float ior_plus_one_in2_tmp = 1.000000;
    vec3 ior_plus_one_out = convert_ior_out + ior_plus_one_in2_tmp;
    vec2 roughness_linear_out = vec2(roughness,anisotropy_switch_out);
    ...
    vec3 ior_divide_out = ior_sub_one_out / ior_plus_one_out;
    ...
    vec3 ior_multiply_out = ior_divide_out * ior_divide_out;
    ...
    vec3 fresnel_mode_switch_out = vec3(0.0);
    NG_switch_color3I(reflectivity, ior_multiply_out, vec3(0.000000, 0.000000, 0.000000), vec3(0.000000, 0.000000, 0.000000), vec3(0.000000, 0.000000, 0.000000), vec3(0.000000, 0.000000, 0.000000), vec3(0.000000, 0.000000, 0.000000), vec3(0.000000, 0.000000, 0.000000), vec3(0.000000, 0.000000, 0.000000), vec3(0.000000, 0.000000, 0.000000), fresnelMode, fresnel_mode_switch_out);
    BSDF reflection_generalized_schlick_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
    mx_generalized_schlick_bsdf_broken(closureData, 1.000000, fresnel_mode_switch_out, vec3(1.000000, 1.000000, 1.000000), reflectivity90, exponent_out, roughness_anisotropic_squared_clamped_out, false, 0.000000, 1.500000, normal, tangent_rotate_normalize_out, 0, 0, reflection_generalized_schlick_bsdf_out);
    BSDF reflection_layer_out = BSDF(vec3(0.0),vec3(1.0));
    mx_multiply_bsdf_color3(closureData, reflection_generalized_schlick_bsdf_out, reflectionTint, reflection_layer_out);
    BSDF transmission_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
    mx_generalized_schlick_bsdf_broken(closureData, 1.000000, fresnel_mode_switch_out, vec3(1.000000, 1.000000, 1.000000), reflectivity90, exponent_out, roughness_anisotropic_squared_clamped_out, false, 0.000000, 1.500000, normal, tangent_rotate_normalize_out, 0, 1, transmission_bsdf_out);
    VDF interior_vdf_out = VDF(vec3(0.0),vec3(1.0));
    mx_anisotropic_vdf(closureData, absorption_vector_out, scatter_vector_out, scatterAnisotropy, interior_vdf_out);
    BSDF transmission_interior_out = BSDF(vec3(0.0),vec3(1.0));
    mx_layer_vdf(closureData, transmission_bsdf_out, interior_vdf_out, transmission_interior_out);
    BSDF transmission_layer_out = BSDF(vec3(0.0),vec3(1.0));
    mx_multiply_bsdf_color3(closureData, transmission_interior_out, transmissionTint, transmission_layer_out);
    BSDF generalized_schlick_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
    mx_layer_bsdf(closureData, reflection_layer_out, transmission_layer_out, generalized_schlick_bsdf_out);
    out1 = generalized_schlick_bsdf_out;
}
...
void main()
{
    ...
    vec3 LamaGeneralizedSchlick_scatterColor_cm_out = vec3(0.0);
    NG_acescg_to_lin_rec709_color3(LamaGeneralizedSchlick_scatterColor_cm_in, LamaGeneralizedSchlick_scatterColor_cm_out);
    surfaceshader LamaGeneralizedSchlickSurface_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = LamaGeneralizedSchlickSurface_opacity;
        ...
        BSDF LamaGeneralizedSchlick_out = BSDF(vec3(0.0),vec3(1.0));
        IMPL_lama_generalized_schlick(closureData, LamaGeneralizedSchlick_reflectionTint_cm_out, LamaGeneralizedSchlick_transmissionTint_cm_out, LamaGeneralizedSchlick_fresnelMode, LamaGeneralizedSchlick_IOR, LamaGeneralizedSchlick_reflectivity_cm_out, LamaGeneralizedSchlick_reflectivity90_cm_out, LamaGeneralizedSchlick_reflectivityProfile, LamaGeneralizedSchlick_roughness, geomprop_Nworld_out1, LamaGeneralizedSchlick_anisotropy, geomprop_Tworld_out1, LamaGeneralizedSchlick_anisotropyRotation, LamaGeneralizedSchlick_absorptionColor_cm_out, LamaGeneralizedSchlick_absorptionRadius, LamaGeneralizedSchlick_scatterColor_cm_out, LamaGeneralizedSchlick_scatterAnisotropy, LamaGeneralizedSchlick_out);
        LamaGeneralizedSchlickSurface_out.color += LamaGeneralizedSchlick_out.response;

        // Compute and apply surface opacity
        {
            LamaGeneralizedSchlickSurface_out.color *= surfaceOpacity;
            LamaGeneralizedSchlickSurface_out.transparency = mix(vec3(1.000000, 1.000000, 1.000000), LamaGeneralizedSchlickSurface_out.transparency, surfaceOpacity);
        }
    }

    material LamaGeneralizedSchlickTest_out = LamaGeneralizedSchlickSurface_out;
    out1 = vec4(LamaGeneralizedSchlickTest_out.color, 1.0);
}
