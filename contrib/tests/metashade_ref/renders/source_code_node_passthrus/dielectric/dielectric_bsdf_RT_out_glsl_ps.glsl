...
uniform float dielectric_RT_weight = 1.000000;
uniform vec3 dielectric_RT_tint = vec3(0.700000, 0.700000, 0.700000);
uniform float dielectric_RT_ior = 1.500000;
uniform vec2 dielectric_RT_roughness = vec2(0.050000, 0.050000);
uniform bool dielectric_RT_retroreflective = false;
uniform float dielectric_RT_thinfilm_thickness = 0.000000;
uniform float dielectric_RT_thinfilm_ior = 1.500000;
uniform int dielectric_RT_distribution = 0;
uniform int dielectric_RT_scatter_mode = 2;
...
void main()
{
    ...
    surfaceshader surface_RT_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF dielectric_RT_out = BSDF(vec3(0.0),vec3(1.0));
            mx_dielectric_bsdf(closureData, dielectric_RT_weight, dielectric_RT_tint, dielectric_RT_ior, dielectric_RT_roughness, dielectric_RT_retroreflective, dielectric_RT_thinfilm_thickness, dielectric_RT_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, dielectric_RT_distribution, dielectric_RT_scatter_mode, dielectric_RT_out);

            surface_RT_out.color += occlusion * dielectric_RT_out.response;
        }
        ...
        BSDF dielectric_RT_out = BSDF(vec3(0.0),vec3(1.0));
        mx_dielectric_bsdf(closureData, dielectric_RT_weight, dielectric_RT_tint, dielectric_RT_ior, dielectric_RT_roughness, dielectric_RT_retroreflective, dielectric_RT_thinfilm_thickness, dielectric_RT_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, dielectric_RT_distribution, dielectric_RT_scatter_mode, dielectric_RT_out);
        surface_RT_out.color += dielectric_RT_out.response;
