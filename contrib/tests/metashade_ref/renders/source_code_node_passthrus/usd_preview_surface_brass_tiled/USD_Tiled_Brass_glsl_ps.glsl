...
uniform vec3 SR_brass1_emissiveColor = vec3(0.000000, 0.000000, 0.000000);
uniform int SR_brass1_useSpecularWorkflow = 0;
uniform vec3 SR_brass1_specularColor = vec3(0.000000, 0.000000, 0.000000);
uniform float SR_brass1_metallic = 1.000000;
uniform float SR_brass1_clearcoat = 0.000000;
uniform float SR_brass1_clearcoatRoughness = 0.010000;
uniform float SR_brass1_opacity = 1.000000;
uniform int SR_brass1_opacityMode = 0;
uniform float SR_brass1_opacityThreshold = 0.000000;
uniform float SR_brass1_ior = 1.500000;
uniform vec3 SR_brass1_normal = vec3(0.000000, 0.000000, 1.000000);
uniform float SR_brass1_displacement = 0.000000;
uniform float SR_brass1_occlusion = 1.000000;
...
void NG_tiledimage_color3(sampler2D file, vec3 default1, vec2 texcoord, vec2 uvtiling, vec2 uvoffset, vec2 realworldimagesize, vec2 realworldtilesize, int filtertype, int framerange, int frameoffset, int frameendaction, out vec3 out1)
{
...
void main()
{
    ...
    material USD_Tiled_Brass_out = SR_brass1_out;
    float outAlpha = clamp(1.0 - dot(USD_Tiled_Brass_out.transparency, vec3(0.3333)), 0.0, 1.0);
    out1 = vec4(USD_Tiled_Brass_out.color, outAlpha);
