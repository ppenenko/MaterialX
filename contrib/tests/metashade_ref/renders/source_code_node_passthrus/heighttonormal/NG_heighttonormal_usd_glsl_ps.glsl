...
uniform int texcoord_index = 0;
uniform float scaled_texcoord_in2 = 10.000000;
uniform sampler2D image_file;
uniform int image_layer = 0;
uniform float image_default = 0.000000;
uniform int image_uaddressmode = 2;
uniform int image_vaddressmode = 2;
uniform int image_filtertype = 1;
uniform int image_framerange = 0;
uniform int image_frameoffset = 0;
uniform int image_frameendaction = 0;
uniform vec2 image_uv_scale = vec2(1.000000, 1.000000);
uniform vec2 image_uv_offset = vec2(0.000000, 0.000000);
uniform float heighttonormal_scale = 1.000000;
uniform float scale_in2 = 2.000000;
uniform vec3 bias_in2 = vec3(-1.000000, -1.000000, -1.000000);
uniform vec3 UsdPreviewSurface_diffuseColor = vec3(0.180000, 0.180000, 0.180000);
uniform vec3 UsdPreviewSurface_emissiveColor = vec3(0.000000, 0.000000, 0.000000);
uniform int UsdPreviewSurface_useSpecularWorkflow = 0;
uniform vec3 UsdPreviewSurface_specularColor = vec3(0.000000, 0.000000, 0.000000);
uniform float UsdPreviewSurface_metallic = 0.000000;
uniform float UsdPreviewSurface_roughness = 0.500000;
uniform float UsdPreviewSurface_clearcoat = 0.000000;
uniform float UsdPreviewSurface_clearcoatRoughness = 0.010000;
uniform float UsdPreviewSurface_opacity = 1.000000;
uniform int UsdPreviewSurface_opacityMode = 0;
uniform float UsdPreviewSurface_opacityThreshold = 0.000000;
uniform float UsdPreviewSurface_ior = 1.500000;
uniform float UsdPreviewSurface_displacement = 0.000000;
uniform float UsdPreviewSurface_occlusion = 1.000000;
...
void IMP_UsdPreviewSurface_surfaceshader(vec3 diffuseColor, vec3 emissiveColor, int useSpecularWorkflow, vec3 specularColor, float metallic, float roughness, float clearcoat, float clearcoatRoughness, float opacity, int opacityMode, float opacityThreshold, float ior, vec3 normal, float displacement, float occlusion, out surfaceshader out1)
{
    ...
    const float transmission_mix_amount_transparent_value2_tmp = 0.000000;
    const float transmission_mix_amount_transparent_in1_tmp = 1.000000;
    float transmission_mix_amount_transparent_out = (opacityThreshold > transmission_mix_amount_transparent_value2_tmp) ? transmission_mix_amount_transparent_in1_tmp : opacity;
    const float metalness_workflow_bsdf_fg_weight_in1_tmp = 1.000000;
    float metalness_workflow_bsdf_fg_weight_out = metalness_workflow_bsdf_fg_weight_in1_tmp * metallic;
    ...
    const float metalness_workflow_bsdf_mix_inv_amount_tmp = 1.000000;
    float metalness_workflow_bsdf_mix_inv_out = metalness_workflow_bsdf_mix_inv_amount_tmp - metallic;
    ...
    const float workflow_selector_bsdf_mix_inv_amount_tmp = 1.000000;
    float workflow_selector_bsdf_mix_inv_out = workflow_selector_bsdf_mix_inv_amount_tmp - use_specular_workflow_float_out;
    float transmission_mix_amount_out = 0.0;
    NG_switch_floatI(transmission_mix_amount_transparent_out, 1.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, opacityMode, transmission_mix_amount_out);
    const float opacity_presence_value2_tmp = 0.000000;
    float opacity_presence_out = (opacityThreshold > opacity_presence_value2_tmp) ? cutout_opacity_out : opacity;
    ...
    float transmission_mix_mix_inv_out = transmission_mix_mix_inv_amount_tmp - transmission_mix_amount_out;
    float transmission_mix_fg_weight_out = diffuse_bsdf_weight_out * transmission_mix_amount_out;
    ...
    surfaceshader surface_constructor_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            mx_generalized_schlick_bsdf_metashade(closureData, clearcoat, coat_F0_out, vec3(1.000000, 1.000000, 1.000000), vec3(1.000000, 1.000000, 1.000000), 5.000000, coat_roughness_out, false, 0.000000, 1.500000, surface_normal_out, geomprop_Tworld_out1, 0, 0, coat_dielectric_bsdf_out);
            BSDF specular_bsdf1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_generalized_schlick_bsdf_metashade(closureData, 1.000000, specularColor, vec3(1.000000, 1.000000, 1.000000), vec3(1.000000, 1.000000, 1.000000), 5.000000, specular_roughness_out, false, 0.000000, 1.500000, surface_normal_out, geomprop_Tworld_out1, 0, 0, specular_bsdf1_out);
            BSDF diffuse_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_oren_nayar_diffuse_bsdf(closureData, transmission_mix_fg_weight_out, diffuseColor, 0.000000, surface_normal_out, false, diffuse_bsdf_out);
            BSDF transmission_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_dielectric_bsdf(closureData, transmission_mix_bg_weight_out, vec3(1.000000, 1.000000, 1.000000), ior, vec2(0.000000, 0.000000), false, 0.000000, 1.500000, surface_normal_out, geomprop_Tworld_out1, 0, 1, transmission_bsdf_out);
            BSDF transmission_mix_add_out = BSDF(vec3(0.0),vec3(1.0));
            mx_add_bsdf(closureData, diffuse_bsdf_out, transmission_bsdf_out, transmission_mix_add_out);
            BSDF specular_workflow_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_bsdf(closureData, specular_bsdf1_out, transmission_mix_add_out, specular_workflow_bsdf_out);
            BSDF workflow_selector_bsdf_fg_mul_out = BSDF(vec3(0.0),vec3(1.0));
            mx_multiply_bsdf_float(closureData, specular_workflow_bsdf_out, use_specular_workflow_float_out, workflow_selector_bsdf_fg_mul_out);
            BSDF metalness_metal_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_conductor_bsdf(closureData, metalness_workflow_bsdf_fg_weight_out, artistic_ior_ior, artistic_ior_extinction, specular_roughness_out, false, 0.000000, 1.500000, surface_normal_out, geomprop_Tworld_out1, 0, metalness_metal_bsdf_out);
            BSDF specular_bsdf2_out = BSDF(vec3(0.0),vec3(1.0));
            mx_generalized_schlick_bsdf_metashade(closureData, 1.000000, F0_out, vec3(1.000000, 1.000000, 1.000000), specular_color_metallic_out, 5.000000, specular_roughness_out, false, 0.000000, 1.500000, surface_normal_out, geomprop_Tworld_out1, 0, 0, specular_bsdf2_out);
            BSDF metalness_specular_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_bsdf(closureData, specular_bsdf2_out, transmission_mix_add_out, metalness_specular_bsdf_out);
            BSDF metalness_workflow_bsdf_bg_mul_out = BSDF(vec3(0.0),vec3(1.0));
            mx_multiply_bsdf_float(closureData, metalness_specular_bsdf_out, metalness_workflow_bsdf_mix_inv_out, metalness_workflow_bsdf_bg_mul_out);
            BSDF metalness_workflow_bsdf_add_out = BSDF(vec3(0.0),vec3(1.0));
            mx_add_bsdf(closureData, metalness_metal_bsdf_out, metalness_workflow_bsdf_bg_mul_out, metalness_workflow_bsdf_add_out);
            BSDF workflow_selector_bsdf_bg_mul_out = BSDF(vec3(0.0),vec3(1.0));
            mx_multiply_bsdf_float(closureData, metalness_workflow_bsdf_add_out, workflow_selector_bsdf_mix_inv_out, workflow_selector_bsdf_bg_mul_out);
            BSDF workflow_selector_bsdf_add_out = BSDF(vec3(0.0),vec3(1.0));
            mx_add_bsdf(closureData, workflow_selector_bsdf_fg_mul_out, workflow_selector_bsdf_bg_mul_out, workflow_selector_bsdf_add_out);
            BSDF coat_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_bsdf(closureData, coat_dielectric_bsdf_out, workflow_selector_bsdf_add_out, coat_bsdf_out);
        ...
        // Compute and apply surface opacity
        {
            ...
            surface_constructor_out.transparency = mix(vec3(1.000000, 1.000000, 1.000000), surface_constructor_out.transparency, surfaceOpacity);
        }
    }
...
void main()
{
    vec2 texcoord_out = vd.texcoord_0.xy;
    vec2 scaled_texcoord_out = texcoord_out * scaled_texcoord_in2;
    float image_out = 0.0;
    mx_image_float(image_file, image_layer, image_default, scaled_texcoord_out, image_uaddressmode, image_vaddressmode, image_filtertype, image_framerange, image_frameoffset, image_frameendaction, image_uv_scale, image_uv_offset, image_out);
    vec3 heighttonormal_out = vec3(0.0);
    mx_heighttonormal_vector3(image_out, heighttonormal_scale, scaled_texcoord_out, heighttonormal_out);
    vec3 scale_out = heighttonormal_out * scale_in2;
    vec3 bias_out = scale_out + bias_in2;
    surfaceshader UsdPreviewSurface_out = surfaceshader(vec3(0.0),vec3(0.0));
    ...
    float outAlpha = clamp(1.0 - dot(UsdPreviewSurface_out.transparency, vec3(0.3333)), 0.0, 1.0);
    usd1 = vec4(UsdPreviewSurface_out.color, outAlpha);
