...
uniform float conty_kulla_sheen_bsdf1_weight = 1.000000;
uniform vec3 conty_kulla_sheen_bsdf1_color = vec3(0.516000, 0.524000, 0.778000);
uniform float conty_kulla_sheen_bsdf1_roughness = 0.300000;
uniform int conty_kulla_sheen_bsdf1_mode = 0;
...
void main()
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF conty_kulla_sheen_bsdf1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_sheen_bsdf(closureData, conty_kulla_sheen_bsdf1_weight, conty_kulla_sheen_bsdf1_color, conty_kulla_sheen_bsdf1_roughness, geomprop_Nworld_out1, conty_kulla_sheen_bsdf1_mode, conty_kulla_sheen_bsdf1_out);

            surface1_out.color += occlusion * conty_kulla_sheen_bsdf1_out.response;
        }

        // Calculate the BSDF transmission for viewing direction
        BSDF conty_kulla_sheen_bsdf1_out = BSDF(vec3(0.0),vec3(1.0));
        surface1_out.color += conty_kulla_sheen_bsdf1_out.response;
