...
uniform float dielectric_weight = 1.000000;
uniform vec3 dielectric_tint = vec3(1.000000, 1.000000, 1.000000);
uniform float dielectric_ior = 1.500000;
uniform vec2 dielectric_roughness = vec2(0.050000, 0.050000);
uniform bool dielectric_retroreflective = false;
uniform float dielectric_thinfilm_thickness = 0.000000;
uniform float dielectric_thinfilm_ior = 1.500000;
uniform int dielectric_distribution = 0;
uniform int dielectric_scatter_mode = 0;
uniform float conductor_weight = 1.000000;
uniform vec3 conductor_ior = vec3(0.183000, 0.421000, 1.373000);
uniform vec3 conductor_extinction = vec3(3.424000, 2.346000, 1.770000);
uniform vec2 conductor_roughness = vec2(0.050000, 0.050000);
uniform bool conductor_retroreflective = false;
uniform float conductor_thinfilm_thickness = 0.000000;
uniform float conductor_thinfilm_ior = 1.500000;
uniform int conductor_distribution = 0;
uniform float custom_layer_scale_top = 0.800000;
uniform float custom_layer_scale_base = 0.300000;
uniform float custom_layer_scale_layer = 0.900000;
...
void NG_scaled_layer(ClosureData closureData, BSDF top, BSDF base, float scale_top, float scale_base, float scale_layer, out BSDF out1)
{
    BSDF top_scaled_out = BSDF(vec3(0.0),vec3(1.0));
    mx_multiply_bsdf_float(closureData, top, scale_top, top_scaled_out);
    BSDF base_scaled_out = BSDF(vec3(0.0),vec3(1.0));
    mx_multiply_bsdf_float(closureData, base, scale_base, base_scaled_out);
    BSDF layer_out = BSDF(vec3(0.0),vec3(1.0));
    mx_layer_bsdf(closureData, top_scaled_out, base_scaled_out, layer_out);
    BSDF layer_scaled_out = BSDF(vec3(0.0),vec3(1.0));
    mx_multiply_bsdf_float(closureData, layer_out, scale_layer, layer_scaled_out);
    out1 = layer_scaled_out;
}
...
void main()
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF dielectric_out = BSDF(vec3(0.0),vec3(1.0));
            mx_dielectric_bsdf(closureData, dielectric_weight, dielectric_tint, dielectric_ior, dielectric_roughness, dielectric_retroreflective, dielectric_thinfilm_thickness, dielectric_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, dielectric_distribution, dielectric_scatter_mode, dielectric_out);
            BSDF conductor_out = BSDF(vec3(0.0),vec3(1.0));
            mx_conductor_bsdf(closureData, conductor_weight, conductor_ior, conductor_extinction, conductor_roughness, conductor_retroreflective, conductor_thinfilm_thickness, conductor_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, conductor_distribution, conductor_out);
            BSDF custom_layer_out = BSDF(vec3(0.0),vec3(1.0));
            NG_scaled_layer(closureData, dielectric_out, conductor_out, custom_layer_scale_top, custom_layer_scale_base, custom_layer_scale_layer, custom_layer_out);

            surface1_out.color += occlusion * custom_layer_out.response;
        }
        ...
        BSDF custom_layer_out = BSDF(vec3(0.0),vec3(1.0));
        NG_scaled_layer(closureData, dielectric_out, conductor_out, custom_layer_scale_top, custom_layer_scale_base, custom_layer_scale_layer, custom_layer_out);
        surface1_out.color += custom_layer_out.response;
