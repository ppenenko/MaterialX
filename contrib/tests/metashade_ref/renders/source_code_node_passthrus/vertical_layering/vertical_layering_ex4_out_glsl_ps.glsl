...
uniform float mybsdf1_diffuse = 0.400000;
uniform vec3 mybsdf1_diffuseColor = vec3(1.000000, 0.100000, 0.100000);
uniform float mybsdf1_specular = 1.000000;
uniform vec3 mybsdf1_specularColor = vec3(1.000000, 1.000000, 1.000000);
uniform float dielectric_brdf1_weight = 1.000000;
uniform vec3 dielectric_brdf1_tint = vec3(1.000000, 1.000000, 1.000000);
uniform float dielectric_brdf1_ior = 1.900000;
uniform vec2 dielectric_brdf1_roughness = vec2(0.050000, 0.050000);
uniform bool dielectric_brdf1_retroreflective = false;
uniform float dielectric_brdf1_thinfilm_thickness = 0.000000;
uniform float dielectric_brdf1_thinfilm_ior = 1.500000;
uniform int dielectric_brdf1_distribution = 0;
uniform int dielectric_brdf1_scatter_mode = 0;
...
void IMP_mybsdf(ClosureData closureData, float diffuse, vec3 diffuseColor, float specular, vec3 specularColor, out BSDF out1)
{
    vec3 geomprop_Nworld_out1 = normalize(vd.normalWorld);
    vec3 geomprop_Tworld_out1 = normalize(vd.tangentWorld);
    BSDF dielectric_brdf1_out = BSDF(vec3(0.0),vec3(1.0));
    mx_dielectric_bsdf(closureData, specular, specularColor, 1.520000, vec2(0.050000, 0.050000), false, 0.000000, 1.500000, geomprop_Nworld_out1, geomprop_Tworld_out1, 0, 0, dielectric_brdf1_out);
    BSDF diffuse_brdf1_out = BSDF(vec3(0.0),vec3(1.0));
    mx_oren_nayar_diffuse_bsdf(closureData, diffuse, diffuseColor, 0.000000, geomprop_Nworld_out1, false, diffuse_brdf1_out);
    BSDF layer1_out = BSDF(vec3(0.0),vec3(1.0));
    mx_layer_bsdf(closureData, dielectric_brdf1_out, diffuse_brdf1_out, layer1_out);
    out1 = layer1_out;
}
...
void main()
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF dielectric_brdf1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_dielectric_bsdf(closureData, dielectric_brdf1_weight, dielectric_brdf1_tint, dielectric_brdf1_ior, dielectric_brdf1_roughness, dielectric_brdf1_retroreflective, dielectric_brdf1_thinfilm_thickness, dielectric_brdf1_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, dielectric_brdf1_distribution, dielectric_brdf1_scatter_mode, dielectric_brdf1_out);
            BSDF mybsdf1_out = BSDF(vec3(0.0),vec3(1.0));
            IMP_mybsdf(closureData, mybsdf1_diffuse, mybsdf1_diffuseColor, mybsdf1_specular, mybsdf1_specularColor, mybsdf1_out);
            BSDF layer1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_bsdf(closureData, dielectric_brdf1_out, mybsdf1_out, layer1_out);

            surface1_out.color += occlusion * layer1_out.response;
        }
        ...
        BSDF layer1_out = BSDF(vec3(0.0),vec3(1.0));
        mx_layer_bsdf(closureData, dielectric_brdf1_out, mybsdf1_out, layer1_out);
        surface1_out.color += layer1_out.response;
