...
uniform float oren_nayar_diffuse_bsdf_weight = 1.000000;
uniform vec3 oren_nayar_diffuse_bsdf_color = vec3(0.180000, 0.180000, 0.180000);
uniform float oren_nayar_diffuse_bsdf_roughness = 0.000000;
uniform bool oren_nayar_diffuse_bsdf_energy_compensation = false;
uniform float dielectric_bsdf_weight = 1.000000;
uniform vec3 dielectric_bsdf_tint = vec3(1.000000, 1.000000, 1.000000);
uniform float dielectric_bsdf_ior = 2.000000;
uniform vec2 dielectric_bsdf_roughness = vec2(0.050000, 0.050000);
uniform bool dielectric_bsdf_retroreflective = false;
uniform float dielectric_bsdf_thinfilm_thickness = 0.000000;
uniform float dielectric_bsdf_thinfilm_ior = 1.500000;
uniform int dielectric_bsdf_distribution = 0;
uniform int dielectric_bsdf_scatter_mode = 0;
...
void main()
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            mx_dielectric_bsdf(closureData, dielectric_bsdf_weight, dielectric_bsdf_tint, dielectric_bsdf_ior, dielectric_bsdf_roughness, dielectric_bsdf_retroreflective, dielectric_bsdf_thinfilm_thickness, dielectric_bsdf_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, dielectric_bsdf_distribution, dielectric_bsdf_scatter_mode, dielectric_bsdf_out);
            BSDF oren_nayar_diffuse_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_oren_nayar_diffuse_bsdf(closureData, oren_nayar_diffuse_bsdf_weight, oren_nayar_diffuse_bsdf_color, oren_nayar_diffuse_bsdf_roughness, geomprop_Nworld_out1, oren_nayar_diffuse_bsdf_energy_compensation, oren_nayar_diffuse_bsdf_out);
            BSDF layer1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_bsdf(closureData, dielectric_bsdf_out, oren_nayar_diffuse_bsdf_out, layer1_out);
