...
uniform float dielectric_bsdf2_ior = 1.500000;
...
uniform float mix_mix = 0.500000;
uniform float surface_opacity = 1.000000;
uniform bool surface_thin_walled = false;
...
void main()
{
    ...
    surfaceshader surface_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = surface_opacity;
        ...
        // Add environment contribution
        {
            ...
            mx_layer_bsdf(closureData, dielectric_bsdf2_out, oren_nayar_diffuse_bsdf1_out, layer_out);
            BSDF mix_out = BSDF(vec3(0.0),vec3(1.0));
            mx_mix_bsdf(closureData, dielectric_bsdf2_out, layer_out, mix_mix, mix_out);

            surface_out.color += occlusion * mix_out.response;
        }
        ...
        BSDF mix_out = BSDF(vec3(0.0),vec3(1.0));
        mx_mix_bsdf(closureData, dielectric_bsdf2_out, layer_out, mix_mix, mix_out);
        surface_out.color += mix_out.response;

        // Compute and apply surface opacity
        {
            surface_out.color *= surfaceOpacity;
            surface_out.transparency = mix(vec3(1.000000, 1.000000, 1.000000), surface_out.transparency, surfaceOpacity);
        }
    }

    out11 = vec4(surface_out.color, 1.0);
}
