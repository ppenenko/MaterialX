...
uniform float diffuse3_weight = 1.000000;
uniform vec3 diffuse3_color = vec3(0.600000, 0.600000, 0.600000);
uniform float diffuse3_roughness = 0.500000;
uniform bool diffuse3_energy_compensation = true;
uniform float surface3_opacity = 1.000000;
uniform bool surface3_thin_walled = false;
...
void main()
{
    ...
    surfaceshader surface3_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = surface3_opacity;
        ...
        // Add environment contribution
        {
            ...
            BSDF diffuse3_out = BSDF(vec3(0.0),vec3(1.0));
            mx_oren_nayar_diffuse_bsdf(closureData, diffuse3_weight, diffuse3_color, diffuse3_roughness, geomprop_Nworld_out1, diffuse3_energy_compensation, diffuse3_out);

            surface3_out.color += occlusion * diffuse3_out.response;
        }

        // Calculate the BSDF transmission for viewing direction
        BSDF diffuse3_out = BSDF(vec3(0.0),vec3(1.0));
        surface3_out.color += diffuse3_out.response;

        // Compute and apply surface opacity
        {
            surface3_out.color *= surfaceOpacity;
            surface3_out.transparency = mix(vec3(1.000000, 1.000000, 1.000000), surface3_out.transparency, surfaceOpacity);
        }
    }

    out31 = vec4(surface3_out.color, 1.0);
}
