...
uniform float diffuse1_weight = 1.000000;
uniform vec3 diffuse1_color = vec3(0.600000, 0.600000, 0.600000);
uniform float diffuse1_roughness = 0.000000;
uniform bool diffuse1_energy_compensation = true;
...
void main()
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF diffuse1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_oren_nayar_diffuse_bsdf(closureData, diffuse1_weight, diffuse1_color, diffuse1_roughness, geomprop_Nworld_out1, diffuse1_energy_compensation, diffuse1_out);

            surface1_out.color += occlusion * diffuse1_out.response;
        }

        // Calculate the BSDF transmission for viewing direction
        BSDF diffuse1_out = BSDF(vec3(0.0),vec3(1.0));
        surface1_out.color += diffuse1_out.response;
