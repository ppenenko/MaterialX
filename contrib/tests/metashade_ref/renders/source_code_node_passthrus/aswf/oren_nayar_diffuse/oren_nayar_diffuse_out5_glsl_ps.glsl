...
uniform float diffuse5_weight = 1.000000;
uniform vec3 diffuse5_color = vec3(0.600000, 0.600000, 0.600000);
uniform float diffuse5_roughness = 1.000000;
uniform bool diffuse5_energy_compensation = true;
uniform float surface5_opacity = 1.000000;
uniform bool surface5_thin_walled = false;
...
void main()
{
    ...
    surfaceshader surface5_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = surface5_opacity;
        ...
        // Add environment contribution
        {
            ...
            BSDF diffuse5_out = BSDF(vec3(0.0),vec3(1.0));
            mx_oren_nayar_diffuse_bsdf(closureData, diffuse5_weight, diffuse5_color, diffuse5_roughness, geomprop_Nworld_out1, diffuse5_energy_compensation, diffuse5_out);

            surface5_out.color += occlusion * diffuse5_out.response;
        }

        // Calculate the BSDF transmission for viewing direction
        BSDF diffuse5_out = BSDF(vec3(0.0),vec3(1.0));
        surface5_out.color += diffuse5_out.response;

        // Compute and apply surface opacity
        {
            surface5_out.color *= surfaceOpacity;
            surface5_out.transparency = mix(vec3(1.000000, 1.000000, 1.000000), surface5_out.transparency, surfaceOpacity);
        }
    }

    out51 = vec4(surface5_out.color, 1.0);
}
