...
uniform float schlick_R_weight = 1.000000;
uniform vec3 schlick_R_color0 = vec3(0.800000, 0.800000, 0.800000);
uniform vec3 schlick_R_color82 = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 schlick_R_color90 = vec3(1.000000, 1.000000, 1.000000);
uniform float schlick_R_exponent = 5.000000;
uniform vec2 schlick_R_roughness = vec2(0.020000, 0.020000);
uniform bool schlick_R_retroreflective = false;
uniform float schlick_R_thinfilm_thickness = 0.000000;
uniform float schlick_R_thinfilm_ior = 1.500000;
uniform int schlick_R_distribution = 0;
uniform int schlick_R_scatter_mode = 0;
uniform float schlick_T_weight = 1.000000;
uniform vec3 schlick_T_color0 = vec3(0.200000, 0.200000, 0.200000);
uniform vec3 schlick_T_color82 = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 schlick_T_color90 = vec3(1.000000, 1.000000, 1.000000);
uniform float schlick_T_exponent = 5.000000;
uniform vec2 schlick_T_roughness = vec2(0.020000, 0.020000);
uniform bool schlick_T_retroreflective = false;
uniform float schlick_T_thinfilm_thickness = 0.000000;
uniform float schlick_T_thinfilm_ior = 1.500000;
uniform int schlick_T_distribution = 0;
uniform int schlick_T_scatter_mode = 1;
uniform float surface_layer_RT_opacity = 1.000000;
uniform bool surface_layer_RT_thin_walled = false;
...
void main()
{
    ...
    surfaceshader surface_layer_RT_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = surface_layer_RT_opacity;
        ...
        // Add environment contribution
        {
            ...
            BSDF schlick_R_out = BSDF(vec3(0.0),vec3(1.0));
            mx_generalized_schlick_bsdf_metashade(closureData, schlick_R_weight, schlick_R_color0, schlick_R_color82, schlick_R_color90, schlick_R_exponent, schlick_R_roughness, schlick_R_retroreflective, schlick_R_thinfilm_thickness, schlick_R_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, schlick_R_distribution, schlick_R_scatter_mode, schlick_R_out);
            BSDF schlick_T_out = BSDF(vec3(0.0),vec3(1.0));
            mx_generalized_schlick_bsdf_metashade(closureData, schlick_T_weight, schlick_T_color0, schlick_T_color82, schlick_T_color90, schlick_T_exponent, schlick_T_roughness, schlick_T_retroreflective, schlick_T_thinfilm_thickness, schlick_T_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, schlick_T_distribution, schlick_T_scatter_mode, schlick_T_out);
            BSDF layer_RT_out2 = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_bsdf(closureData, schlick_R_out, schlick_T_out, layer_RT_out2);

            surface_layer_RT_out.color += occlusion * layer_RT_out2.response;
        }
        ...
        BSDF layer_RT_out2 = BSDF(vec3(0.0),vec3(1.0));
        mx_layer_bsdf(closureData, schlick_R_out, schlick_T_out, layer_RT_out2);
        surface_layer_RT_out.color += layer_RT_out2.response;

        // Compute and apply surface opacity
        {
            surface_layer_RT_out.color *= surfaceOpacity;
            surface_layer_RT_out.transparency = mix(vec3(1.000000, 1.000000, 1.000000), surface_layer_RT_out.transparency, surfaceOpacity);
        }
    }

    layer_RT_out1 = vec4(surface_layer_RT_out.color, 1.0);
}
