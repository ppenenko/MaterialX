...
uniform int texcoord1_x_index = 0;
uniform vec2 varying_ior_test1_roughness = vec2(0.100000, 0.100000);
...
void IMP_foo_surface(float ior, vec2 roughness, out surfaceshader out1)
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF dielectric_brdf_1_layer_top_out = BSDF(vec3(0.0),vec3(1.0));
            mx_dielectric_bsdf(closureData, 1.000000, vec3(1.000000, 1.000000, 1.000000), ior, roughness, false, 0.000000, 1.500000, geomprop_Nworld_out1, geomprop_Tworld_out1, 0, 0, dielectric_brdf_1_layer_top_out);
            BSDF dielectric_btdf_1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_dielectric_bsdf(closureData, 1.000000, vec3(1.000000, 1.000000, 1.000000), ior, roughness, false, 0.000000, 1.500000, geomprop_Nworld_out1, geomprop_Tworld_out1, 0, 1, dielectric_btdf_1_out);
            BSDF dielectric_brdf_1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_bsdf(closureData, dielectric_brdf_1_layer_top_out, dielectric_btdf_1_out, dielectric_brdf_1_out);

            surface1_out.color += occlusion * dielectric_brdf_1_out.response;
        }
        ...
        BSDF dielectric_brdf_1_out = BSDF(vec3(0.0),vec3(1.0));
        mx_layer_bsdf(closureData, dielectric_brdf_1_layer_top_out, dielectric_btdf_1_out, dielectric_brdf_1_out);
        surface1_out.color += dielectric_brdf_1_out.response;
...
void main()
{
    ...
    float texcoord1_x_out = texcoord1_out[texcoord1_x_index];
    surfaceshader varying_ior_test1_out = surfaceshader(vec3(0.0),vec3(0.0));
    IMP_foo_surface(texcoord1_x_out, varying_ior_test1_roughness, varying_ior_test1_out);
    material varying_ior_test1_mtrl_out = varying_ior_test1_out;
    out1 = vec4(varying_ior_test1_mtrl_out.color, 1.0);
}
