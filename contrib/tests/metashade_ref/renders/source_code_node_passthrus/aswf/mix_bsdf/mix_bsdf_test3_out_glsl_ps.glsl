...
uniform vec3 shader1_albedo = vec3(0.800000, 0.200000, 0.100000);
uniform vec3 shader1_subsurface = vec3(0.100000, 0.100000, 0.800000);
uniform float shader1_subsurface_weight = 0.000000;
uniform vec3 shader1_emission = vec3(0.000000, 0.000000, 0.000000);
uniform float shader1_opacity = 1.000000;
...
void IMP_substratebsdf(ClosureData closureData, vec3 albedo, vec3 subsurface, float subsurface_weight, out BSDF out1)
{
    const float mix1_fg_weight_in1_tmp = 1.000000;
    float mix1_fg_weight_out = mix1_fg_weight_in1_tmp * subsurface_weight;
    vec3 geomprop_Nworld_out1 = normalize(vd.normalWorld);
    const float mix1_mix_inv_amount_tmp = 1.000000;
    float mix1_mix_inv_out = mix1_mix_inv_amount_tmp - subsurface_weight;
    const float mix1_bg_weight_in1_tmp = 1.000000;
    float mix1_bg_weight_out = mix1_bg_weight_in1_tmp * mix1_mix_inv_out;
    BSDF diffuse_out = BSDF(vec3(0.0),vec3(1.0));
    mx_oren_nayar_diffuse_bsdf(closureData, mix1_fg_weight_out, albedo, 0.000000, geomprop_Nworld_out1, false, diffuse_out);
    BSDF subsurface2_out = BSDF(vec3(0.0),vec3(1.0));
    mx_translucent_bsdf(closureData, mix1_bg_weight_out, subsurface, geomprop_Nworld_out1, subsurface2_out);
    BSDF mix1_add_out = BSDF(vec3(0.0),vec3(1.0));
    mx_add_bsdf(closureData, diffuse_out, subsurface2_out, mix1_add_out);
    out1 = mix1_add_out;
}
...
void IMP_substrateshader(vec3 albedo, vec3 subsurface, float subsurface_weight, vec3 emission, float opacity, out surfaceshader out1)
{
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF substrate1_out = BSDF(vec3(0.0),vec3(1.0));
            IMP_substratebsdf(closureData, albedo, subsurface, subsurface_weight, substrate1_out);

            surface1_out.color += occlusion * substrate1_out.response;
        }

        // Add surface emission
        {
            ClosureData closureData = makeClosureData(CLOSURE_TYPE_EMISSION, L, V, N, P, occlusion);
            EDF edf1_out = EDF(0.0);
            mx_uniform_edf(closureData, emission, edf1_out);
            surface1_out.color += edf1_out;
        }
        ...
        BSDF substrate1_out = BSDF(vec3(0.0),vec3(1.0));
        IMP_substratebsdf(closureData, albedo, subsurface, subsurface_weight, substrate1_out);
        surface1_out.color += substrate1_out.response;
...
void main()
{
    surfaceshader shader1_out = surfaceshader(vec3(0.0),vec3(0.0));
    IMP_substrateshader(shader1_albedo, shader1_subsurface, shader1_subsurface_weight, shader1_emission, shader1_opacity, shader1_out);
    out11 = vec4(shader1_out.color, 1.0);
}
