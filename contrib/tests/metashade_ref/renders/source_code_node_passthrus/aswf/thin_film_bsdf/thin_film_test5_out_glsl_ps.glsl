...
uniform vec3 dielectric_brdf_2_tint = vec3(0.000000, 1.000000, 0.000000);
uniform float dielectric_brdf_2_ior = 2.500000;
uniform vec2 dielectric_brdf_2_roughness = vec2(0.300000, 0.300000);
uniform bool dielectric_brdf_2_retroreflective = false;
uniform float dielectric_brdf_2_thinfilm_thickness = 550.000000;
...
uniform vec3 dielectric_brdf_1_tint = vec3(1.000000, 0.000000, 0.000000);
uniform float dielectric_brdf_1_ior = 1.500000;
uniform vec2 dielectric_brdf_1_roughness = vec2(0.300000, 0.300000);
uniform bool dielectric_brdf_1_retroreflective = false;
uniform float dielectric_brdf_1_thinfilm_thickness = 550.000000;
...
uniform float mix_bsdf_1_mix = 0.500000;
...
void main()
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF mix_bsdf_1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_mix_bsdf(closureData, dielectric_brdf_2_out, dielectric_brdf_1_out, mix_bsdf_1_mix, mix_bsdf_1_out);

            surface1_out.color += occlusion * mix_bsdf_1_out.response;
        }
        ...
        BSDF mix_bsdf_1_out = BSDF(vec3(0.0),vec3(1.0));
        mx_mix_bsdf(closureData, dielectric_brdf_2_out, dielectric_brdf_1_out, mix_bsdf_1_mix, mix_bsdf_1_out);
        surface1_out.color += mix_bsdf_1_out.response;
