...
uniform float burley_brdf1_weight = 1.000000;
uniform vec3 burley_brdf1_color = vec3(0.600000, 0.600000, 0.600000);
uniform float burley_brdf1_roughness = 0.200000;
...
void main()
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF burley_brdf1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_burley_diffuse_bsdf(closureData, burley_brdf1_weight, burley_brdf1_color, burley_brdf1_roughness, geomprop_Nworld_out1, burley_brdf1_out);

            surface1_out.color += occlusion * burley_brdf1_out.response;
        }

        // Calculate the BSDF transmission for viewing direction
        BSDF burley_brdf1_out = BSDF(vec3(0.0),vec3(1.0));
        surface1_out.color += burley_brdf1_out.response;
