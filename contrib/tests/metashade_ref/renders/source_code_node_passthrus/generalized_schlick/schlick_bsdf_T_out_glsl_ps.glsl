...
uniform float surface_T_opacity = 1.000000;
uniform bool surface_T_thin_walled = false;
...
void main()
{
    ...
    surfaceshader surface_T_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = surface_T_opacity;
        ...
        // Add environment contribution
        {
            ...
            surface_T_out.color += occlusion * schlick_T_out.response;
        }
        ...
        BSDF schlick_T_out = BSDF(vec3(0.0),vec3(1.0));
        mx_generalized_schlick_bsdf_metashade(closureData, schlick_T_weight, schlick_T_color0, schlick_T_color82, schlick_T_color90, schlick_T_exponent, schlick_T_roughness, schlick_T_retroreflective, schlick_T_thinfilm_thickness, schlick_T_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, schlick_T_distribution, schlick_T_scatter_mode, schlick_T_out);
        surface_T_out.color += schlick_T_out.response;

        // Compute and apply surface opacity
        {
            surface_T_out.color *= surfaceOpacity;
            surface_T_out.transparency = mix(vec3(1.000000, 1.000000, 1.000000), surface_T_out.transparency, surfaceOpacity);
        }
    }

    T_out1 = vec4(surface_T_out.color, 1.0);
}
