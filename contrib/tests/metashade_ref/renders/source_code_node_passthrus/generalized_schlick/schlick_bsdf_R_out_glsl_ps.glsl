...
uniform float surface_R_opacity = 1.000000;
uniform bool surface_R_thin_walled = false;
...
void main()
{
    ...
    surfaceshader surface_R_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = surface_R_opacity;
        ...
        // Add environment contribution
        {
            ...
            surface_R_out.color += occlusion * schlick_R_out.response;
        }
        ...
        BSDF schlick_R_out = BSDF(vec3(0.0),vec3(1.0));
        mx_generalized_schlick_bsdf_metashade(closureData, schlick_R_weight, schlick_R_color0, schlick_R_color82, schlick_R_color90, schlick_R_exponent, schlick_R_roughness, schlick_R_retroreflective, schlick_R_thinfilm_thickness, schlick_R_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, schlick_R_distribution, schlick_R_scatter_mode, schlick_R_out);
        surface_R_out.color += schlick_R_out.response;

        // Compute and apply surface opacity
        {
            surface_R_out.color *= surfaceOpacity;
            surface_R_out.transparency = mix(vec3(1.000000, 1.000000, 1.000000), surface_R_out.transparency, surfaceOpacity);
        }
    }

    R_out1 = vec4(surface_R_out.color, 1.0);
}
