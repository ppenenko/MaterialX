...
uniform float schlick_R3_weight = 1.000000;
uniform vec3 schlick_R3_color0 = vec3(0.800000, 0.800000, 0.800000);
uniform vec3 schlick_R3_color82 = vec3(0.200000, 0.200000, 0.800000);
uniform vec3 schlick_R3_color90 = vec3(0.800000, 0.800000, 0.800000);
uniform float schlick_R3_exponent = 5.000000;
uniform vec2 schlick_R3_roughness = vec2(0.020000, 0.020000);
uniform bool schlick_R3_retroreflective = false;
uniform float schlick_R3_thinfilm_thickness = 0.000000;
uniform float schlick_R3_thinfilm_ior = 1.500000;
uniform int schlick_R3_distribution = 0;
uniform int schlick_R3_scatter_mode = 0;
uniform float surface_R3_opacity = 1.000000;
uniform bool surface_R3_thin_walled = false;
...
void main()
{
    ...
    surfaceshader surface_R3_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = surface_R3_opacity;
        ...
        // Add environment contribution
        {
            ...
            BSDF schlick_R3_out = BSDF(vec3(0.0),vec3(1.0));
            mx_generalized_schlick_bsdf_metashade(closureData, schlick_R3_weight, schlick_R3_color0, schlick_R3_color82, schlick_R3_color90, schlick_R3_exponent, schlick_R3_roughness, schlick_R3_retroreflective, schlick_R3_thinfilm_thickness, schlick_R3_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, schlick_R3_distribution, schlick_R3_scatter_mode, schlick_R3_out);

            surface_R3_out.color += occlusion * schlick_R3_out.response;
        }
        ...
        BSDF schlick_R3_out = BSDF(vec3(0.0),vec3(1.0));
        mx_generalized_schlick_bsdf_metashade(closureData, schlick_R3_weight, schlick_R3_color0, schlick_R3_color82, schlick_R3_color90, schlick_R3_exponent, schlick_R3_roughness, schlick_R3_retroreflective, schlick_R3_thinfilm_thickness, schlick_R3_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, schlick_R3_distribution, schlick_R3_scatter_mode, schlick_R3_out);
        surface_R3_out.color += schlick_R3_out.response;

        // Compute and apply surface opacity
        {
            surface_R3_out.color *= surfaceOpacity;
            surface_R3_out.transparency = mix(vec3(1.000000, 1.000000, 1.000000), surface_R3_out.transparency, surfaceOpacity);
        }
    }

    R3_out1 = vec4(surface_R3_out.color, 1.0);
}
