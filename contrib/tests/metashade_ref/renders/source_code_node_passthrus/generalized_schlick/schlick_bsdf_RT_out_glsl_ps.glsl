...
uniform float schlick_RT_weight = 1.000000;
uniform vec3 schlick_RT_color0 = vec3(0.200000, 0.200000, 0.200000);
uniform vec3 schlick_RT_color82 = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 schlick_RT_color90 = vec3(1.000000, 1.000000, 1.000000);
uniform float schlick_RT_exponent = 5.000000;
uniform vec2 schlick_RT_roughness = vec2(0.020000, 0.020000);
uniform bool schlick_RT_retroreflective = false;
uniform float schlick_RT_thinfilm_thickness = 0.000000;
uniform float schlick_RT_thinfilm_ior = 1.500000;
uniform int schlick_RT_distribution = 0;
uniform int schlick_RT_scatter_mode = 2;
uniform float surface_RT_opacity = 1.000000;
uniform bool surface_RT_thin_walled = false;
...
void main()
{
    ...
    surfaceshader surface_RT_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = surface_RT_opacity;
        ...
        // Add environment contribution
        {
            ...
            BSDF schlick_RT_out = BSDF(vec3(0.0),vec3(1.0));
            mx_generalized_schlick_bsdf_metashade(closureData, schlick_RT_weight, schlick_RT_color0, schlick_RT_color82, schlick_RT_color90, schlick_RT_exponent, schlick_RT_roughness, schlick_RT_retroreflective, schlick_RT_thinfilm_thickness, schlick_RT_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, schlick_RT_distribution, schlick_RT_scatter_mode, schlick_RT_out);

            surface_RT_out.color += occlusion * schlick_RT_out.response;
        }
        ...
        BSDF schlick_RT_out = BSDF(vec3(0.0),vec3(1.0));
        mx_generalized_schlick_bsdf_metashade(closureData, schlick_RT_weight, schlick_RT_color0, schlick_RT_color82, schlick_RT_color90, schlick_RT_exponent, schlick_RT_roughness, schlick_RT_retroreflective, schlick_RT_thinfilm_thickness, schlick_RT_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, schlick_RT_distribution, schlick_RT_scatter_mode, schlick_RT_out);
        surface_RT_out.color += schlick_RT_out.response;

        // Compute and apply surface opacity
        {
            surface_RT_out.color *= surfaceOpacity;
            surface_RT_out.transparency = mix(vec3(1.000000, 1.000000, 1.000000), surface_RT_out.transparency, surfaceOpacity);
        }
    }

    RT_out1 = vec4(surface_RT_out.color, 1.0);
}
