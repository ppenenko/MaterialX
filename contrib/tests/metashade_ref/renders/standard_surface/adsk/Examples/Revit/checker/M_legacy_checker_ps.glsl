...
uniform vec3 legacy_checker_color3_color1 = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 legacy_checker_color3_color2 = vec3(0.840948, 0.000000, 0.000000);
uniform vec2 legacy_checker_color3_realworld_scale = vec2(1.400000, 0.300000);
uniform float legacy_checker_color3_soften = 0.030000;
uniform bool legacy_checker_color3_adjust_soften_y = true;
uniform vec2 legacy_checker_color3_realworld_offset = vec2(-0.800000, 0.300000);
uniform float legacy_checker_color3_rotation_angle = -20.000000;
uniform bool legacy_checker_color3_tile_x = true;
uniform bool legacy_checker_color3_tile_y = true;
...
void NG_checkerboard_color3(vec3 color1, vec3 color2, vec2 uvtiling, vec2 uvoffset, vec2 texcoord, out vec3 out1)
{
    vec2 N_mtlxmult_out = texcoord * uvtiling;
    vec2 N_mtlxsubtract_out = N_mtlxmult_out - uvoffset;
    vec2 N_mtlxfloor_out = floor(N_mtlxsubtract_out);
    const vec2 N_mtlxdotproduct_in2_tmp = vec2(1.000000, 1.000000);
    float N_mtlxdotproduct_out = dot(N_mtlxfloor_out, N_mtlxdotproduct_in2_tmp);
    const float N_modulo_in2_tmp = 2.000000;
    float N_modulo_out = mx_mod(N_mtlxdotproduct_out, N_modulo_in2_tmp);
    vec3 N_mtlxmix_out = mix(color2, color1, N_modulo_out);
    out1 = N_mtlxmix_out;
}
...
void NG_legacy_checker_color3(vec2 texcoord, vec3 color1, vec3 color2, vec2 realworld_scale, float soften, bool adjust_soften_y, vec2 realworld_offset, float rotation_angle, bool tile_x, bool tile_y, out vec3 output_color3, out float output_alpha, out vec4 output_color4)
{
    const float invert_rotation_amount_tmp = 0.000000;
    float invert_rotation_out = invert_rotation_amount_tmp - rotation_angle;
    vec2 subtract_offset_out = texcoord - realworld_offset;
    const vec2 adj_size_2x2_in1_tmp = vec2(2.000000, 2.000000);
    vec2 adj_size_2x2_out = adj_size_2x2_in1_tmp / realworld_scale;
    ...
    const float half_soften_in2_tmp = 0.500000;
    float half_soften_out = soften * half_soften_in2_tmp;
    vec2 rotate_coord_out = vec2(0.0);
    mx_rotate_vector2(subtract_offset_out, invert_rotation_out, rotate_coord_out);
    float ratio_out = separate_size_outx / separate_size_outy;
    vec2 trim_coord_out = rotate_coord_out / realworld_scale;
    vec3 checkerboard_color3_out = vec3(0.0);
    NG_checkerboard_color3(color1, color2, adj_size_2x2_out, vec2(0.000000, 0.000000), rotate_coord_out, checkerboard_color3_out);
    float adj_softeny_out = soften * ratio_out;
    float adj_halfsofteny_out = half_soften_out * ratio_out;
    float separate_trim_coord_outx = 0.0;
    float separate_trim_coord_outy = 0.0;
    NG_separate2_vector2(trim_coord_out, separate_trim_coord_outx, separate_trim_coord_outy);
    const float modulo_coord_in2_tmp = 1.000000;
    vec2 modulo_coord_out = mx_mod(trim_coord_out, modulo_coord_in2_tmp);
    const bool adjsofty_enable2_value2_tmp = true;
    float adjsofty_enable2_out = (adjust_soften_y == adjsofty_enable2_value2_tmp) ? adj_softeny_out : soften;
    const bool adjsofty_enable1_value2_tmp = true;
    float adjsofty_enable1_out = (adjust_soften_y == adjsofty_enable1_value2_tmp) ? adj_halfsofteny_out : half_soften_out;
    ...
    float ifgreater_x_1_out = (separate_trim_coord_outx > ifgreater_x_1_value2_tmp) ? ifgreater_x_1_in1_tmp : ifgreater_x_1_in2_tmp;
    ...
    float ifgreater_y_1_out = (separate_trim_coord_outy > ifgreater_y_1_value2_tmp) ? ifgreater_y_1_in1_tmp : ifgreater_y_1_in2_tmp;
    float separate_coord_outx = 0.0;
    float separate_coord_outy = 0.0;
    NG_separate2_vector2(modulo_coord_out, separate_coord_outx, separate_coord_outy);
    const float ifgreater_x_0_value2_tmp = 0.000000;
    const float ifgreater_x_0_in2_tmp = 0.000000;
    float ifgreater_x_0_out = (separate_trim_coord_outx > ifgreater_x_0_value2_tmp) ? ifgreater_x_1_out : ifgreater_x_0_in2_tmp;
    const float ifgreater_y_0_value2_tmp = 0.000000;
    const float ifgreater_y_0_in2_tmp = 0.000000;
    float ifgreater_y_0_out = (separate_trim_coord_outy > ifgreater_y_0_value2_tmp) ? ifgreater_y_1_out : ifgreater_y_0_in2_tmp;
    float add_hs_x_out = separate_coord_outx + half_soften_out;
    float subtract_hs_x_out = separate_coord_outx - half_soften_out;
    float add_hs_y_out = separate_coord_outy + adjsofty_enable1_out;
    float subtract_hs_y_out = separate_coord_outy - adjsofty_enable1_out;
    ...
    float floor1_out = floor(add_hs_x_out);
    const float modulo1_in2_tmp = 1.000000;
    float modulo1_out = mx_mod(add_hs_x_out, modulo1_in2_tmp);
    float floor2_out = floor(subtract_hs_x_out);
    const float modulo2_in2_tmp = 1.000000;
    float modulo2_out = mx_mod(subtract_hs_x_out, modulo2_in2_tmp);
    float floor3_out = floor(add_hs_y_out);
    const float modulo3_in2_tmp = 1.000000;
    float modulo3_out = mx_mod(add_hs_y_out, modulo3_in2_tmp);
    float floor4_out = floor(subtract_hs_y_out);
    const float modulo4_in2_tmp = 1.000000;
    float modulo4_out = mx_mod(subtract_hs_y_out, modulo4_in2_tmp);
    float combine_alpha_out = check_trim_x_out * check_trim_y_out;
    const float multiply1_in2_tmp = 0.500000;
    float multiply1_out = floor1_out * multiply1_in2_tmp;
    ...
    float multiply2_out = floor2_out * multiply2_in2_tmp;
    const float subtract2_in2_tmp = 0.500000;
    float subtract2_out = modulo2_out - subtract2_in2_tmp;
    const float multiply3_in2_tmp = 0.500000;
    float multiply3_out = floor3_out * multiply3_in2_tmp;
    const float subtract3_in2_tmp = 0.500000;
    float subtract3_out = modulo3_out - subtract3_in2_tmp;
    const float multiply4_in2_tmp = 0.500000;
    float multiply4_out = floor4_out * multiply4_in2_tmp;
    const float subtract4_in2_tmp = 0.500000;
    float subtract4_out = modulo4_out - subtract4_in2_tmp;
    const float max1_in1_tmp = 0.000000;
    float max1_out = max(max1_in1_tmp, subtract1_out);
    const float max2_in1_tmp = 0.000000;
    float max2_out = max(max2_in1_tmp, subtract2_out);
    const float max3_in1_tmp = 0.000000;
    float max3_out = max(max3_in1_tmp, subtract3_out);
    const float max4_in1_tmp = 0.000000;
    float max4_out = max(max4_in1_tmp, subtract4_out);
    float add1_out = multiply1_out + max1_out;
    float add2_out = multiply2_out + max2_out;
    float add3_out = multiply3_out + max3_out;
    float add4_out = multiply4_out + max4_out;
    float subtract_sinteg_x_out = add1_out - add2_out;
    float subtract_sinteg_y_out = add3_out - add4_out;
    float divide_x_soften_out = subtract_sinteg_x_out / soften;
    float divide_y_soften_out = subtract_sinteg_y_out / adjsofty_enable2_out;
    const float invert1_amount_tmp = 1.000000;
    float invert1_out = invert1_amount_tmp - divide_x_soften_out;
    float multiply_xy_out = divide_x_soften_out * divide_y_soften_out;
    const float invert2_amount_tmp = 1.000000;
    float invert2_out = invert2_amount_tmp - divide_y_soften_out;
    float multiply_inv_xy_out = invert1_out * invert2_out;
    float add_xy_invxy_out = multiply_xy_out + multiply_inv_xy_out;
    vec3 mix_soften_color_out = mix(color1, color2, add_xy_invxy_out);
    const float soften_enable_value2_tmp = 0.000000;
    vec3 soften_enable_out = (soften > soften_enable_value2_tmp) ? mix_soften_color_out : checkerboard_color3_out;
    vec3 rgb_alpha_mask_out = soften_enable_out * combine_alpha_out;
    float separate_rgb_outr = 0.0;
    float separate_rgb_outg = 0.0;
    float separate_rgb_outb = 0.0;
    NG_separate3_color3(rgb_alpha_mask_out, separate_rgb_outr, separate_rgb_outg, separate_rgb_outb);
    vec4 combine_color4_out = vec4(separate_rgb_outr,separate_rgb_outg,separate_rgb_outb,combine_alpha_out);
    output_color3 = rgb_alpha_mask_out;
    output_alpha = combine_alpha_out;
    output_color4 = combine_color4_out;
}
