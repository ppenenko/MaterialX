...
uniform vec3 archviz_glazing_surface_albedo = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 archviz_glazing_surface_cutout = vec3(1.000000, 1.000000, 1.000000);
uniform float archviz_glazing_surface_roughness = 0.000000;
uniform float archviz_glazing_surface_anisotropy = 0.000000;
uniform float archviz_glazing_surface_rotation = 0.000000;
uniform vec3 archviz_glazing_glazing_transmission_color = vec3(0.688410, 0.743471, 0.784766);
uniform float archviz_glazing_glazing_transmission_roughness = 0.547723;
uniform vec3 archviz_glazing_glazing_f0 = vec3(0.318997, 0.333939, 0.347338);
uniform bool archviz_glazing_glazing_backface_culling = true;
...
void adsk_NG_adsk_glazing(vec3 surface_albedo, vec3 surface_normal, vec3 surface_cutout, float surface_roughness, float surface_anisotropy, float surface_rotation, vec3 glazing_transmission_color, float glazing_transmission_roughness, vec3 glazing_f0, bool glazing_backface_culling, out surfaceshader out1)
{
    const float oneminus_surfaceaniso0_in1_tmp = 1.000000;
    float oneminus_surfaceaniso0_out = oneminus_surfaceaniso0_in1_tmp - surface_anisotropy;
    vec3 lum_glazing_f0_out = vec3(0.0);
    mx_luminance_color3(glazing_f0, vec3(0.272229, 0.674082, 0.053689), lum_glazing_f0_out);
    ...
    float transmission_extra_roughness_value_out = glazing_transmission_roughness - surface_roughness;
    vec3 geomprop_Nworld_out1 = normalize(vd.normalWorld);
    const int surface_cutout_value_index_tmp = 0;
    float surface_cutout_value_out = surface_cutout[surface_cutout_value_index_tmp];
    vec3 geomprop_Tworld_out1 = normalize(vd.tangentWorld);
    float sqrt1minus_surfaceaniso0_out = sqrt(oneminus_surfaceaniso0_out);
    const int lum_glazing_f0_float_index_tmp = 0;
    float lum_glazing_f0_float_out = lum_glazing_f0_out[lum_glazing_f0_float_index_tmp];
    const vec3 oneminus_glazingf0_in1_tmp = vec3(1.000000, 1.000000, 1.000000);
    vec3 oneminus_glazingf0_out = oneminus_glazingf0_in1_tmp - lum_glazing_f0_out;
    ...
    vec3 opacity_value_out = vec3(0.0);
    mx_backface_util(glazing_backface_culling, surface_cutout_value_out, opacity_value_out);
    float specular_roughness_value_out = sqrt1minus_surfaceaniso0_out * surface_roughness;
    float sqrt_glazingf0_out = sqrt(lum_glazing_f0_float_out);
    vec3 glazing_transmission_color_value_out = glazing_transmission_color / oneminus_glazingf0_out;
    const float oneplus_sqrtglazingf0_in1_tmp = 1.000000;
    float oneplus_sqrtglazingf0_out = oneplus_sqrtglazingf0_in1_tmp + sqrt_glazingf0_out;
    const float oneminus_sqrtglazingf0_in1_tmp = 1.000000;
    float oneminus_sqrtglazingf0_out = oneminus_sqrtglazingf0_in1_tmp - sqrt_glazingf0_out;
    float specular_IOR_value_out = oneplus_sqrtglazingf0_out / oneminus_sqrtglazingf0_out;
    surfaceshader standard_surface0_out = surfaceshader(vec3(0.0),vec3(0.0));
    NG_standard_surface_surfaceshader_100(1.000000, vec3(0.800000, 0.800000, 0.800000), 0.000000, 0.000000, 1.000000, surface_albedo, specular_roughness_value_out, specular_IOR_value_out, specular_anisotropy_value_out, specular_rotation_value_out, 1.000000, glazing_transmission_color_value_out, 0.000000, vec3(0.000000, 0.000000, 0.000000), 0.000000, 0.000000, transmission_extra_roughness_value_out, 0.000000, vec3(1.000000, 1.000000, 1.000000), vec3(1.000000, 1.000000, 1.000000), 1.000000, 0.000000, 0.000000, vec3(1.000000, 1.000000, 1.000000), 0.300000, 0.000000, vec3(1.000000, 1.000000, 1.000000), 0.100000, 0.000000, 0.000000, 1.500000, geomprop_Nworld_out1, 0.000000, 0.000000, 0.000000, 1.500000, 0.000000, vec3(1.000000, 1.000000, 1.000000), opacity_value_out, true, surface_normal, geomprop_Tworld_out1, standard_surface0_out);
...
{
    vec3 geomprop_Nworld_out1 = normalize(vd.normalWorld);
    surfaceshader archviz_glazing_out = surfaceshader(vec3(0.0),vec3(0.0));
    adsk_NG_adsk_glazing(archviz_glazing_surface_albedo, geomprop_Nworld_out1, archviz_glazing_surface_cutout, archviz_glazing_surface_roughness, archviz_glazing_surface_anisotropy, archviz_glazing_surface_rotation, archviz_glazing_glazing_transmission_color, archviz_glazing_glazing_transmission_roughness, archviz_glazing_glazing_f0, archviz_glazing_glazing_backface_culling, archviz_glazing_out);
    material Glazing_test_out = archviz_glazing_out;
    float outAlpha = clamp(1.0 - dot(Glazing_test_out.transparency, vec3(0.3333)), 0.0, 1.0);
    out1 = vec4(Glazing_test_out.color, outAlpha);
