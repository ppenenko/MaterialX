...
uniform sampler2D test_transparency_file;
uniform int test_transparency_layer = 0;
uniform float test_transparency_default = 0.000000;
uniform int test_transparency_uaddressmode = 2;
uniform int test_transparency_vaddressmode = 2;
uniform int test_transparency_filtertype = 1;
uniform int test_transparency_framerange = 0;
uniform int test_transparency_frameoffset = 0;
uniform int test_transparency_frameendaction = 0;
uniform vec2 test_transparency_uv_scale = vec2(1.000000, 1.000000);
uniform vec2 test_transparency_uv_offset = vec2(0.000000, 0.000000);
uniform sampler2D test_cutout_file;
uniform int test_cutout_layer = 0;
uniform vec3 test_cutout_default = vec3(0.000000, 0.000000, 0.000000);
uniform int test_cutout_uaddressmode = 2;
uniform int test_cutout_vaddressmode = 2;
uniform int test_cutout_filtertype = 1;
uniform int test_cutout_framerange = 0;
uniform int test_cutout_frameoffset = 0;
uniform int test_cutout_frameendaction = 0;
uniform vec2 test_cutout_uv_scale = vec2(1.000000, 1.000000);
uniform vec2 test_cutout_uv_offset = vec2(0.000000, 0.000000);
uniform vec3 SR_legacy_generic_diffuse_color = vec3(0.500000, 0.900000, 0.200000);
uniform bool SR_legacy_generic_diffuse_image_enable = false;
uniform vec3 SR_legacy_generic_diffuse_image = vec3(1.000000, 1.000000, 1.000000);
uniform float SR_legacy_generic_diffuse_image_fade = 1.000000;
uniform bool SR_legacy_generic_tint_enable = false;
uniform vec3 SR_legacy_generic_tint_color = vec3(0.900000, 0.300000, 0.200000);
uniform float SR_legacy_generic_glossiness = 0.750000;
uniform bool SR_legacy_generic_metallic_highlights = false;
uniform bool SR_legacy_generic_reflectivity_enable = true;
uniform float SR_legacy_generic_reflectivity = 0.070000;
uniform bool SR_legacy_generic_transparency_enable = false;
uniform float SR_legacy_generic_transparency_amount = 0.980000;
uniform bool SR_legacy_generic_transparency_image_enable = false;
uniform float SR_legacy_generic_transparency_image_fade = 1.000000;
uniform int SR_legacy_generic_refraction = 4;
uniform float SR_legacy_generic_custom_refraction = 1.800000;
uniform float SR_legacy_generic_translucency = 0.000000;
uniform bool SR_legacy_generic_bump_enable = false;
uniform float SR_legacy_generic_bump_amount = 1.000000;
uniform bool SR_legacy_generic_cutout_enable = false;
uniform bool SR_legacy_generic_emission_enable = false;
uniform vec3 SR_legacy_generic_emission_color = vec3(1.000000, 1.000000, 1.000000);
uniform int SR_legacy_generic_luminance = 1;
uniform float SR_legacy_generic_custom_luminance = 500.000000;
uniform int SR_legacy_generic_color_temperature = 5;
uniform float SR_legacy_generic_custom_color_temperature = 4800.000000;
...
void NG_transformnormal_w2t_vector3(vec3 world_normal, out vec3 tangent_normal)
{
    vec3 tangent_vector3_out = normalize(vd.tangentWorld);
    vec3 bitangent_vector3_out = normalize(vd.bitangentWorld);
    vec3 normal_vector4_out = normalize(vd.normalWorld);
    float dotproduct_vx_out = dot(world_normal, tangent_vector3_out);
    float dotproduct_vy_out = dot(world_normal, bitangent_vector3_out);
    float dotproduct_vz_out = dot(world_normal, normal_vector4_out);
    vec3 combine3_vector3_out = vec3(dotproduct_vx_out,dotproduct_vy_out,dotproduct_vz_out);
    vec3 normalize_vector3_out = normalize(combine3_vector3_out);
    const float add_vector3FA_in2_tmp = 1.000000;
    vec3 add_vector3FA_out = normalize_vector3_out + add_vector3FA_in2_tmp;
    const float divide_vector3FA_in2_tmp = 2.000000;
    vec3 divide_vector3FA_out = add_vector3FA_out / divide_vector3FA_in2_tmp;
    tangent_normal = divide_vector3FA_out;
}
...
void NG_legacy_generic(vec3 diffuse_color, bool diffuse_image_enable, vec3 diffuse_image, float diffuse_image_fade, bool tint_enable, vec3 tint_color, float glossiness, bool metallic_highlights, bool reflectivity_enable, float reflectivity, bool transparency_enable, float transparency_amount, bool transparency_image_enable, float transparency_image, float transparency_image_fade, int refraction, float custom_refraction, float translucency, bool bump_enable, vec3 normal_bump, float bump_amount, bool cutout_enable, vec3 cutout_image, bool emission_enable, vec3 emission_color, int luminance, float custom_luminance, int color_temperature, float custom_color_temperature, out surfaceshader out1)
{
    vec3 diffuse_image_mix_out = mix(diffuse_color, diffuse_image, diffuse_image_fade);
    const float refl_extr_outlow_tmp = 0.000000;
    const float refl_extr_inlow_tmp = 0.100000;
    const float refl_extr_outhigh_tmp = 1.000000;
    const float refl_extr_inhigh_tmp = 1.000000;
    float refl_extr_out = refl_extr_outlow_tmp + (reflectivity - refl_extr_inlow_tmp) * (refl_extr_outhigh_tmp - refl_extr_outlow_tmp) / (refl_extr_inhigh_tmp - refl_extr_inlow_tmp);
    const bool reflectivity_selection1_value2_tmp = true;
    const float reflectivity_selection1_in1_tmp = 1.000000;
    const float reflectivity_selection1_in2_tmp = 0.000000;
    float reflectivity_selection1_out = (reflectivity_enable == reflectivity_selection1_value2_tmp) ? reflectivity_selection1_in1_tmp : reflectivity_selection1_in2_tmp;
    const float invert_gloss_amount_tmp = 1.000000;
    float invert_gloss_out = invert_gloss_amount_tmp - glossiness;
    float convert_ior_float_out = float(refraction);
    float transparency_image_mix_out = mix(transparency_amount, transparency_image, transparency_image_fade);
    vec3 geomprop_Nworld_out1 = normalize(vd.normalWorld);
    float convert_em_float_out = float(luminance);
    float convert_K_float_out = float(color_temperature);
    const bool cutout_selection_value2_tmp = true;
    const vec3 cutout_selection_in2_tmp = vec3(1.000000, 1.000000, 1.000000);
    vec3 cutout_selection_out = (cutout_enable == cutout_selection_value2_tmp) ? cutout_image : cutout_selection_in2_tmp;
    vec3 transformnormal_w2t_vector3_1_tangent_normal = vec3(0.0);
    NG_transformnormal_w2t_vector3(normal_bump, transformnormal_w2t_vector3_1_tangent_normal);
    vec3 geomprop_Tworld_out1 = normalize(vd.tangentWorld);
    vec3 geomprop_Bworld_out1 = normalize(vd.bitangentWorld);
    const bool diffuse_image_selection_value2_tmp = true;
    vec3 diffuse_image_selection_out = (diffuse_image_enable == diffuse_image_selection_value2_tmp) ? diffuse_image_mix_out : diffuse_color;
    const float metallic_component_value2_tmp = 0.100000;
    const float metallic_component_in2_tmp = 0.000000;
    float metallic_component_out = (reflectivity > metallic_component_value2_tmp) ? refl_extr_out : metallic_component_in2_tmp;
    const float modulo_ior_in2_tmp = 5.000000;
    float modulo_ior_out = mx_mod(convert_ior_float_out, modulo_ior_in2_tmp);
    const float divide_ior_in2_tmp = 5.000000;
    float divide_ior_out = convert_ior_float_out / divide_ior_in2_tmp;
    const bool transparency_image_selection_value2_tmp = true;
    float transparency_image_selection_out = (transparency_image_enable == transparency_image_selection_value2_tmp) ? transparency_image_mix_out : transparency_amount;
    const float modulo_luminance_in2_tmp = 5.000000;
    float modulo_luminance_out = mx_mod(convert_em_float_out, modulo_luminance_in2_tmp);
    const float divide_luminance_in2_tmp = 5.000000;
    float divide_luminance_out = convert_em_float_out / divide_luminance_in2_tmp;
    const float modulo_K_in2_tmp = 5.000000;
    float modulo_K_out = mx_mod(convert_K_float_out, modulo_K_in2_tmp);
    const float divide_K_in2_tmp = 5.000000;
    float divide_K_out = convert_K_float_out / divide_K_in2_tmp;
    vec3 neutral_normalmap_out = vec3(0.0);
    mx_normalmap_float(vec3(0.500000, 0.500000, 1.000000), 1.000000, geomprop_Nworld_out1, geomprop_Tworld_out1, geomprop_Bworld_out1, neutral_normalmap_out);
    vec3 normalmap1_out = vec3(0.0);
    mx_normalmap_float(transformnormal_w2t_vector3_1_tangent_normal, bump_amount, geomprop_Nworld_out1, geomprop_Tworld_out1, geomprop_Bworld_out1, normalmap1_out);
    vec3 tint_mult_out = diffuse_image_selection_out * tint_color;
    const bool reflectivity_selection2_value2_tmp = true;
    const float reflectivity_selection2_in2_tmp = 0.000000;
    float reflectivity_selection2_out = (reflectivity_enable == reflectivity_selection2_value2_tmp) ? metallic_component_out : reflectivity_selection2_in2_tmp;
    float switch_ior1_out = 0.0;
    NG_switch_float(1.000000, 1.330000, 1.360000, 1.460000, 1.520000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, modulo_ior_out, switch_ior1_out);
    float switch_ior2_out = 0.0;
    NG_switch_float(2.300000, custom_refraction, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, modulo_ior_out, switch_ior2_out);
    const bool transparency_selection_value2_tmp = true;
    const float transparency_selection_in2_tmp = 0.000000;
    float transparency_selection_out = (transparency_enable == transparency_selection_value2_tmp) ? transparency_image_selection_out : transparency_selection_in2_tmp;
    float switch_luminance1_out = 0.0;
    NG_switch_float(10.000000, 100.000000, 140.000000, 200.000000, 250.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, modulo_luminance_out, switch_luminance1_out);
    float switch_luminance2_out = 0.0;
    NG_switch_float(1300.000000, 2500.000000, 10000.000000, 20000.000000, 210000.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, modulo_luminance_out, switch_luminance2_out);
    float switch_luminance3_out = 0.0;
    NG_switch_float(custom_luminance, 2.400000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, modulo_luminance_out, switch_luminance3_out);
    float switch_K1_out = 0.0;
    NG_switch_float(1850.000000, 2800.000000, 3400.000000, 4100.000000, 5000.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, modulo_K_out, switch_K1_out);
    float switch_K2_out = 0.0;
    NG_switch_float(6000.000000, 6420.000000, 9320.000000, custom_color_temperature, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, modulo_K_out, switch_K2_out);
    const bool bump_selection_value2_tmp = true;
    vec3 bump_selection_out = (bump_enable == bump_selection_value2_tmp) ? normalmap1_out : neutral_normalmap_out;
    const bool tint_selection_value2_tmp = true;
    vec3 tint_selection_out = (tint_enable == tint_selection_value2_tmp) ? tint_mult_out : diffuse_image_selection_out;
    float switch_ior_out = 0.0;
    NG_switch_float(switch_ior1_out, switch_ior2_out, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, divide_ior_out, switch_ior_out);
    float switch_luminance_out = 0.0;
    NG_switch_float(switch_luminance1_out, switch_luminance2_out, switch_luminance3_out, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, divide_luminance_out, switch_luminance_out);
    float switch_K_out = 0.0;
    NG_switch_float(switch_K1_out, switch_K2_out, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, divide_K_out, switch_K_out);
    const bool metallic_highlights_selection_value2_tmp = true;
    const vec3 metallic_highlights_selection_in2_tmp = vec3(1.000000, 1.000000, 1.000000);
    vec3 metallic_highlights_selection_out = (metallic_highlights == metallic_highlights_selection_value2_tmp) ? tint_selection_out : metallic_highlights_selection_in2_tmp;
    const bool emission_value_value2_tmp = true;
    const float emission_value_in2_tmp = 0.000000;
    float emission_value_out = (emission_enable == emission_value_value2_tmp) ? switch_luminance_out : emission_value_in2_tmp;
    vec3 blackbody_out = vec3(0.0);
    mx_blackbody(switch_K_out, blackbody_out);
    ...
    vec3 multiply_col_K_out = emission_color * blackbody_out;
    surfaceshader standard_surface_out = surfaceshader(vec3(0.0),vec3(0.0));
    NG_standard_surface_surfaceshader_100(1.000000, tint_selection_out, 0.000000, reflectivity_selection2_out, reflectivity_selection1_out, metallic_highlights_selection_out, invert_gloss_out, switch_ior_out, 0.000000, 0.000000, transparency_selection_out, vec3(1.000000, 1.000000, 1.000000), 0.000000, vec3(0.000000, 0.000000, 0.000000), 0.000000, 0.000000, 0.000000, translucency, tint_selection_out, vec3(1.000000, 1.000000, 1.000000), 1.000000, 0.000000, 0.000000, vec3(1.000000, 1.000000, 1.000000), 0.300000, 0.000000, vec3(1.000000, 1.000000, 1.000000), 0.100000, 0.000000, 0.000000, 1.500000, geomprop_Nworld_out1, 0.000000, 0.000000, 0.000000, 1.500000, emission_value_scaled_out, multiply_col_K_out, cutout_selection_out, false, bump_selection_out, geomprop_Tworld_out1, standard_surface_out);
...
void main()
{
    ...
    float test_transparency_out = 0.0;
    mx_image_float(test_transparency_file, test_transparency_layer, test_transparency_default, geomprop_UV0_out1, test_transparency_uaddressmode, test_transparency_vaddressmode, test_transparency_filtertype, test_transparency_framerange, test_transparency_frameoffset, test_transparency_frameendaction, test_transparency_uv_scale, test_transparency_uv_offset, test_transparency_out);
    vec3 test_cutout_out = vec3(0.0);
    mx_image_color3(test_cutout_file, test_cutout_layer, test_cutout_default, geomprop_UV0_out1, test_cutout_uaddressmode, test_cutout_vaddressmode, test_cutout_filtertype, test_cutout_framerange, test_cutout_frameoffset, test_cutout_frameendaction, test_cutout_uv_scale, test_cutout_uv_offset, test_cutout_out);
    surfaceshader SR_legacy_generic_out = surfaceshader(vec3(0.0),vec3(0.0));
    NG_legacy_generic(SR_legacy_generic_diffuse_color, SR_legacy_generic_diffuse_image_enable, SR_legacy_generic_diffuse_image, SR_legacy_generic_diffuse_image_fade, SR_legacy_generic_tint_enable, SR_legacy_generic_tint_color, SR_legacy_generic_glossiness, SR_legacy_generic_metallic_highlights, SR_legacy_generic_reflectivity_enable, SR_legacy_generic_reflectivity, SR_legacy_generic_transparency_enable, SR_legacy_generic_transparency_amount, SR_legacy_generic_transparency_image_enable, test_transparency_out, SR_legacy_generic_transparency_image_fade, SR_legacy_generic_refraction, SR_legacy_generic_custom_refraction, SR_legacy_generic_translucency, SR_legacy_generic_bump_enable, geomprop_Nworld_out1, SR_legacy_generic_bump_amount, SR_legacy_generic_cutout_enable, test_cutout_out, SR_legacy_generic_emission_enable, SR_legacy_generic_emission_color, SR_legacy_generic_luminance, SR_legacy_generic_custom_luminance, SR_legacy_generic_color_temperature, SR_legacy_generic_custom_color_temperature, SR_legacy_generic_out);
    material M_legacy_generic_out = SR_legacy_generic_out;
    float outAlpha = clamp(1.0 - dot(M_legacy_generic_out.transparency, vec3(0.3333)), 0.0, 1.0);
    out1 = vec4(M_legacy_generic_out.color, outAlpha);
