...
uniform sampler2D test_granite_bump_file;
uniform vec2 test_granite_bump_realworld_offset = vec2(0.000000, 0.000000);
uniform vec2 test_granite_bump_realworld_scale = vec2(0.250000, 0.250000);
uniform vec2 test_granite_bump_uv_offset = vec2(0.000000, 0.000000);
uniform vec2 test_granite_bump_uv_scale = vec2(1.000000, 1.000000);
uniform float test_granite_bump_rotation_angle = 0.000000;
uniform float test_granite_bump_depth = 0.050000;
uniform int test_granite_bump_uaddressmode = 2;
uniform int test_granite_bump_vaddressmode = 2;
uniform sampler2D test_marble_bump_file;
uniform vec2 test_marble_bump_realworld_offset = vec2(0.000000, 0.000000);
uniform vec2 test_marble_bump_realworld_scale = vec2(0.250000, 0.250000);
uniform vec2 test_marble_bump_uv_offset = vec2(0.000000, 0.000000);
uniform vec2 test_marble_bump_uv_scale = vec2(1.000000, 1.000000);
uniform float test_marble_bump_rotation_angle = 0.000000;
uniform float test_marble_bump_depth = 0.050000;
uniform int test_marble_bump_uaddressmode = 2;
uniform int test_marble_bump_vaddressmode = 2;
uniform sampler2D test_stonewall_bump_file;
uniform vec2 test_stonewall_bump_realworld_offset = vec2(0.000000, 0.000000);
uniform vec2 test_stonewall_bump_realworld_scale = vec2(0.250000, 0.250000);
uniform vec2 test_stonewall_bump_uv_offset = vec2(0.000000, 0.000000);
uniform vec2 test_stonewall_bump_uv_scale = vec2(1.000000, 1.000000);
uniform float test_stonewall_bump_rotation_angle = 0.000000;
uniform float test_stonewall_bump_depth = 0.050000;
uniform int test_stonewall_bump_uaddressmode = 2;
uniform int test_stonewall_bump_vaddressmode = 2;
uniform sampler2D height_map1_stone_file;
uniform vec2 height_map1_stone_realworld_offset = vec2(0.000000, 0.000000);
uniform vec2 height_map1_stone_realworld_scale = vec2(0.250000, 0.250000);
uniform vec2 height_map1_stone_uv_offset = vec2(0.000000, 0.000000);
uniform vec2 height_map1_stone_uv_scale = vec2(1.000000, 1.000000);
uniform float height_map1_stone_rotation_angle = 0.000000;
uniform float height_map1_stone_depth = 0.150000;
uniform int height_map1_stone_uaddressmode = 2;
uniform int height_map1_stone_vaddressmode = 2;
uniform sampler2D height_map2_stone_file;
uniform vec2 height_map2_stone_realworld_offset = vec2(0.000000, 0.000000);
uniform vec2 height_map2_stone_realworld_scale = vec2(0.250000, 0.250000);
uniform vec2 height_map2_stone_uv_offset = vec2(0.000000, 0.000000);
uniform vec2 height_map2_stone_uv_scale = vec2(1.000000, 1.000000);
uniform float height_map2_stone_rotation_angle = 0.000000;
uniform float height_map2_stone_depth = 0.150000;
uniform int height_map2_stone_uaddressmode = 2;
uniform int height_map2_stone_vaddressmode = 2;
uniform vec3 SR_legacy_stone_color = vec3(0.500000, 0.500000, 0.500000);
uniform int SR_legacy_stone_finish = 2;
uniform bool SR_legacy_stone_tint_enable = false;
uniform vec3 SR_legacy_stone_tint_color = vec3(0.950000, 0.500000, 0.500000);
uniform bool SR_legacy_stone_finish_bump_enable = true;
uniform int SR_legacy_stone_finish_bump = 0;
uniform bool SR_legacy_stone_relief_pattern_enable = false;
uniform sampler2D SR_legacy_stone_finish_granite_filename;
uniform sampler2D SR_legacy_stone_finish_wall_filename;
uniform sampler2D SR_legacy_stone_finish_marble_filename;
uniform float SR_legacy_stone_finish_amount = 1.000000;
uniform float SR_legacy_stone_finish_scale = 1.000000;
...
void NG_legacy_stone(vec2 texcoord, vec3 color, int finish, bool tint_enable, vec3 tint_color, bool finish_bump_enable, int finish_bump, vec3 normal_finish_granite, vec3 normal_finish_marble, vec3 normal_finish_wall, vec3 normal_custom_finish, bool relief_pattern_enable, vec3 normal_relief_pattern, sampler2D finish_granite_filename, sampler2D finish_wall_filename, sampler2D finish_marble_filename, float finish_amount, float finish_scale, out surfaceshader out1)
{
    vec3 tint_mult_out = color * tint_color;
    float switch_finish_out = 0.0;
    NG_switch_floatI(0.010000, 0.050000, 0.150000, 0.400000, 0.700000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, finish, switch_finish_out);
    vec3 geomprop_Nworld_out1 = normalize(vd.normalWorld);
    const float invert_scale_in1_tmp = 1.000000;
    float invert_scale_out = invert_scale_in1_tmp / finish_scale;
    const float bump_amt_granite_in1_tmp = 0.030000;
    float bump_amt_granite_out = bump_amt_granite_in1_tmp * finish_amount;
    vec3 geomprop_Tworld_out1 = normalize(vd.tangentWorld);
    const float bump_amt_wall_in1_tmp = 0.100000;
    float bump_amt_wall_out = bump_amt_wall_in1_tmp * finish_amount;
    const float bump_amt_marble_in1_tmp = 0.020000;
    float bump_amt_marble_out = bump_amt_marble_in1_tmp * finish_amount;
    vec3 transformnormal_w2t_vector3_1_tangent_normal = vec3(0.0);
    NG_transformnormal_w2t_vector3(normal_custom_finish, transformnormal_w2t_vector3_1_tangent_normal);
    ...
    vec2 finish_scale_adj_out = vec2(invert_scale_out,invert_scale_out);
    vec3 normalmap_neutral_out = vec3(0.0);
    mx_normalmap_float(vec3(0.500000, 0.500000, 1.000000), 1.000000, geomprop_Nworld_out1, geomprop_Tworld_out1, geomprop_Bworld_out1, normalmap_neutral_out);
    vec3 normalmap1_out = vec3(0.0);
    mx_normalmap_float(transformnormal_w2t_vector3_1_tangent_normal, finish_amount, geomprop_Nworld_out1, geomprop_Tworld_out1, geomprop_Bworld_out1, normalmap1_out);
    vec3 height_map_marble_out = vec3(0.0);
    adsk_NG_adsk_height_map(finish_marble_filename, vec2(0.000000, 0.000000), vec2(15.000000, 15.000000), vec2(0.000000, 0.000000), finish_scale_adj_out, 0.000000, bump_amt_marble_out, 2, 2, texcoord, geomprop_Nworld_out1, geomprop_Tworld_out1, height_map_marble_out);
    vec3 height_map_wall_out = vec3(0.0);
    adsk_NG_adsk_height_map(finish_wall_filename, vec2(0.000000, 0.000000), vec2(100.000000, 100.000000), vec2(0.000000, 0.000000), finish_scale_adj_out, 0.000000, bump_amt_wall_out, 2, 2, texcoord, geomprop_Nworld_out1, geomprop_Tworld_out1, height_map_wall_out);
    vec3 height_map_granite_out = vec3(0.0);
    adsk_NG_adsk_height_map(finish_granite_filename, vec2(0.000000, 0.000000), vec2(15.000000, 15.000000), vec2(0.000000, 0.000000), finish_scale_adj_out, 0.000000, bump_amt_granite_out, 2, 2, texcoord, geomprop_Nworld_out1, geomprop_Tworld_out1, height_map_granite_out);
    vec3 switch_finish_bump_out = vec3(0.0);
    NG_switch_vector3I(height_map_granite_out, height_map_wall_out, height_map_marble_out, normalmap1_out, vec3(0.000000, 0.000000, 0.000000), vec3(0.000000, 0.000000, 0.000000), vec3(0.000000, 0.000000, 0.000000), vec3(0.000000, 0.000000, 0.000000), vec3(0.000000, 0.000000, 0.000000), vec3(0.000000, 0.000000, 0.000000), finish_bump, switch_finish_bump_out);
    const bool finish_bump_selection_value2_tmp = true;
    vec3 finish_bump_selection_out = (finish_bump_enable == finish_bump_selection_value2_tmp) ? switch_finish_bump_out : normalmap_neutral_out;
    surfaceshader standard_surface_out = surfaceshader(vec3(0.0),vec3(0.0));
    NG_standard_surface_surfaceshader_100(1.000000, tint_selection_out, 0.000000, 0.000000, 1.000000, vec3(1.000000, 1.000000, 1.000000), switch_finish_out, 1.500000, 0.000000, 0.000000, 0.000000, vec3(1.000000, 1.000000, 1.000000), 0.000000, vec3(0.000000, 0.000000, 0.000000), 0.000000, 0.000000, 0.000000, 0.000000, vec3(1.000000, 1.000000, 1.000000), vec3(1.000000, 1.000000, 1.000000), 1.000000, 0.000000, 0.000000, vec3(1.000000, 1.000000, 1.000000), 0.300000, 0.000000, vec3(1.000000, 1.000000, 1.000000), 0.100000, 0.000000, 0.000000, 1.500000, geomprop_Nworld_out1, 0.000000, 0.000000, 0.000000, 1.500000, 0.000000, vec3(1.000000, 1.000000, 1.000000), vec3(1.000000, 1.000000, 1.000000), false, finish_bump_selection_out, geomprop_Tworld_out1, standard_surface_out);
...
void main()
{
    ...
    vec3 geomprop_Nworld_out1 = normalize(vd.normalWorld);
    vec3 geomprop_Tworld_out1 = normalize(vd.tangentWorld);
    vec3 test_granite_bump_out = vec3(0.0);
    adsk_NG_adsk_height_map(test_granite_bump_file, test_granite_bump_realworld_offset, test_granite_bump_realworld_scale, test_granite_bump_uv_offset, test_granite_bump_uv_scale, test_granite_bump_rotation_angle, test_granite_bump_depth, test_granite_bump_uaddressmode, test_granite_bump_vaddressmode, geomprop_UV0_out1, geomprop_Nworld_out1, geomprop_Tworld_out1, test_granite_bump_out);
    vec3 test_marble_bump_out = vec3(0.0);
    adsk_NG_adsk_height_map(test_marble_bump_file, test_marble_bump_realworld_offset, test_marble_bump_realworld_scale, test_marble_bump_uv_offset, test_marble_bump_uv_scale, test_marble_bump_rotation_angle, test_marble_bump_depth, test_marble_bump_uaddressmode, test_marble_bump_vaddressmode, geomprop_UV0_out1, geomprop_Nworld_out1, geomprop_Tworld_out1, test_marble_bump_out);
    vec3 test_stonewall_bump_out = vec3(0.0);
    adsk_NG_adsk_height_map(test_stonewall_bump_file, test_stonewall_bump_realworld_offset, test_stonewall_bump_realworld_scale, test_stonewall_bump_uv_offset, test_stonewall_bump_uv_scale, test_stonewall_bump_rotation_angle, test_stonewall_bump_depth, test_stonewall_bump_uaddressmode, test_stonewall_bump_vaddressmode, geomprop_UV0_out1, geomprop_Nworld_out1, geomprop_Tworld_out1, test_stonewall_bump_out);
    vec3 height_map1_stone_out = vec3(0.0);
    adsk_NG_adsk_height_map(height_map1_stone_file, height_map1_stone_realworld_offset, height_map1_stone_realworld_scale, height_map1_stone_uv_offset, height_map1_stone_uv_scale, height_map1_stone_rotation_angle, height_map1_stone_depth, height_map1_stone_uaddressmode, height_map1_stone_vaddressmode, geomprop_UV0_out1, geomprop_Nworld_out1, geomprop_Tworld_out1, height_map1_stone_out);
    vec3 height_map2_stone_out = vec3(0.0);
    adsk_NG_adsk_height_map(height_map2_stone_file, height_map2_stone_realworld_offset, height_map2_stone_realworld_scale, height_map2_stone_uv_offset, height_map2_stone_uv_scale, height_map2_stone_rotation_angle, height_map2_stone_depth, height_map2_stone_uaddressmode, height_map2_stone_vaddressmode, geomprop_UV0_out1, geomprop_Nworld_out1, geomprop_Tworld_out1, height_map2_stone_out);
    surfaceshader SR_legacy_stone_out = surfaceshader(vec3(0.0),vec3(0.0));
    NG_legacy_stone(geomprop_UV0_out1, SR_legacy_stone_color, SR_legacy_stone_finish, SR_legacy_stone_tint_enable, SR_legacy_stone_tint_color, SR_legacy_stone_finish_bump_enable, SR_legacy_stone_finish_bump, test_granite_bump_out, test_marble_bump_out, test_stonewall_bump_out, height_map1_stone_out, SR_legacy_stone_relief_pattern_enable, height_map2_stone_out, SR_legacy_stone_finish_granite_filename, SR_legacy_stone_finish_wall_filename, SR_legacy_stone_finish_marble_filename, SR_legacy_stone_finish_amount, SR_legacy_stone_finish_scale, SR_legacy_stone_out);
    material M_legacy_stone_out = SR_legacy_stone_out;
    out1 = vec4(M_legacy_stone_out.color, 1.0);
}
