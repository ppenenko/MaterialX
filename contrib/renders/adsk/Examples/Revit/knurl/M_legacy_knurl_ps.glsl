...
uniform vec2 legacy_knurl_realworld_scale_unit_in1 = vec2(0.100000, 0.200000);
uniform float legacy_knurl_realworld_scale_unit_in2 = 2.540000;
uniform float legacy_knurl_amount = 0.250000;
uniform float legacy_knurl_rotation_angle = 45.000000;
uniform vec2 legacy_knurl_realworld_offset = vec2(0.000000, 0.000000);
uniform bool legacy_knurl_diagonal = true;
uniform float SR_legacy_knurl_base = 1.000000;
uniform vec3 SR_legacy_knurl_base_color = vec3(0.800000, 0.800000, 0.800000);
uniform float SR_legacy_knurl_diffuse_roughness = 0.000000;
uniform float SR_legacy_knurl_metalness = 1.000000;
uniform float SR_legacy_knurl_specular = 1.000000;
uniform vec3 SR_legacy_knurl_specular_color = vec3(1.000000, 1.000000, 1.000000);
uniform float SR_legacy_knurl_specular_roughness = 0.100000;
uniform float SR_legacy_knurl_specular_IOR = 1.500000;
uniform float SR_legacy_knurl_specular_anisotropy = 0.000000;
uniform float SR_legacy_knurl_specular_rotation = 0.000000;
uniform float SR_legacy_knurl_transmission = 0.000000;
uniform vec3 SR_legacy_knurl_transmission_color = vec3(1.000000, 1.000000, 1.000000);
uniform float SR_legacy_knurl_transmission_depth = 0.000000;
uniform vec3 SR_legacy_knurl_transmission_scatter = vec3(0.000000, 0.000000, 0.000000);
uniform float SR_legacy_knurl_transmission_scatter_anisotropy = 0.000000;
uniform float SR_legacy_knurl_transmission_dispersion = 0.000000;
uniform float SR_legacy_knurl_transmission_extra_roughness = 0.000000;
uniform float SR_legacy_knurl_subsurface = 0.400000;
uniform vec3 SR_legacy_knurl_subsurface_color = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 SR_legacy_knurl_subsurface_radius = vec3(1.000000, 1.000000, 1.000000);
uniform float SR_legacy_knurl_subsurface_scale = 1.000000;
uniform float SR_legacy_knurl_subsurface_anisotropy = 0.000000;
uniform float SR_legacy_knurl_sheen = 0.000000;
uniform vec3 SR_legacy_knurl_sheen_color = vec3(1.000000, 1.000000, 1.000000);
uniform float SR_legacy_knurl_sheen_roughness = 0.300000;
uniform float SR_legacy_knurl_coat = 0.000000;
uniform vec3 SR_legacy_knurl_coat_color = vec3(1.000000, 1.000000, 1.000000);
uniform float SR_legacy_knurl_coat_roughness = 0.100000;
uniform float SR_legacy_knurl_coat_anisotropy = 0.000000;
uniform float SR_legacy_knurl_coat_rotation = 0.000000;
uniform float SR_legacy_knurl_coat_IOR = 1.500000;
uniform float SR_legacy_knurl_coat_affect_color = 0.000000;
uniform float SR_legacy_knurl_coat_affect_roughness = 0.000000;
uniform float SR_legacy_knurl_thin_film_thickness = 0.000000;
uniform float SR_legacy_knurl_thin_film_IOR = 1.500000;
uniform float SR_legacy_knurl_emission = 0.000000;
uniform vec3 SR_legacy_knurl_emission_color = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 SR_legacy_knurl_opacity = vec3(1.000000, 1.000000, 1.000000);
uniform bool SR_legacy_knurl_thin_walled = false;
...
void NG_legacy_knurl(vec2 texcoord, float amount, float rotation_angle, vec2 realworld_scale, vec2 realworld_offset, bool diagonal, out vec3 out_rgb, out vec3 out_vec3)
{
    vec2 offset_uv_out = texcoord - realworld_offset;
    const float lagacy_rot_direction_amount_tmp = 0.000000;
    float lagacy_rot_direction_out = lagacy_rot_direction_amount_tmp - rotation_angle;
    const float invert_amount_amount_tmp = 1.000000;
    float invert_amount_out = invert_amount_amount_tmp - amount;
    const float diag_adjustment_in2_tmp = 45.000000;
    float diag_adjustment_out = rotation_angle - diag_adjustment_in2_tmp;
    ...
    vec2 rotate2d_pattern_out = vec2(0.0);
    mx_rotate_vector2(offset_uv_out, lagacy_rot_direction_out, rotate2d_pattern_out);
    const float amount_to_degrees_in2_tmp = 90.000000;
    float amount_to_degrees_out = invert_amount_out * amount_to_degrees_in2_tmp;
    const bool diag_select2_value2_tmp = true;
    float diag_select2_out = (diagonal == diag_select2_value2_tmp) ? diag_adjustment_out : rotation_angle;
    vec2 divide1_out = rotate2d_pattern_out / realworld_scale;
    vec3 rotate_Y1_out = vec3(0.0);
    mx_rotate_vector3(vec3(1.000000, 0.000000, 0.000000), amount_to_degrees_out, vec3(0.000000, 1.000000, 0.000000), rotate_Y1_out);
    const float invert_180_amount_tmp = 180.000000;
    float invert_180_out = invert_180_amount_tmp - amount_to_degrees_out;
    vec3 rotate_X2_out = vec3(0.0);
    mx_rotate_vector3(vec3(0.000000, -1.000000, 0.000000), amount_to_degrees_out, vec3(1.000000, 0.000000, 0.000000), rotate_X2_out);
    const float uv_modulo_in2_tmp = 1.000000;
    vec2 uv_modulo_out = mx_mod(divide1_out, uv_modulo_in2_tmp);
    vec3 rotate_Y2_out = vec3(0.0);
    mx_rotate_vector3(vec3(1.000000, 0.000000, 0.000000), invert_180_out, vec3(0.000000, 1.000000, 0.000000), rotate_Y2_out);
    vec3 rotate_X1_out = vec3(0.0);
    mx_rotate_vector3(vec3(0.000000, -1.000000, 0.000000), invert_180_out, vec3(1.000000, 0.000000, 0.000000), rotate_X1_out);
    const float diag_scale_in2_tmp = 1.414210;
    vec2 diag_scale_out = uv_modulo_out * diag_scale_in2_tmp;
    vec2 rotate_knurl_out = vec2(0.0);
    mx_rotate_vector2(diag_scale_out, 45.000000, rotate_knurl_out);
    const vec2 diag_shift_in2_tmp = vec2(0.500000, 0.500000);
    vec2 diag_shift_out = rotate_knurl_out + diag_shift_in2_tmp;
    const float modulo_knurl_in2_tmp = 1.000000;
    vec2 modulo_knurl_out = mx_mod(diag_shift_out, modulo_knurl_in2_tmp);
    const bool diag_select1_value2_tmp = true;
    vec2 diag_select1_out = (diagonal == diag_select1_value2_tmp) ? modulo_knurl_out : uv_modulo_out;
    float separate_uv_outx = 0.0;
    float separate_uv_outy = 0.0;
    NG_separate2_vector2(diag_select1_out, separate_uv_outx, separate_uv_outy);
    const float invert_x_amount_tmp = 1.000000;
    float invert_x_out = invert_x_amount_tmp - separate_uv_outx;
    const float ifgreater_half_x_value2_tmp = 0.500000;
    vec3 ifgreater_half_x_out = (separate_uv_outx > ifgreater_half_x_value2_tmp) ? rotate_Y1_out : rotate_Y2_out;
    const float ifgreater_xy_in1_tmp = 1.000000;
    const float ifgreater_xy_in2_tmp = 0.000000;
    float ifgreater_xy_out = (separate_uv_outx > separate_uv_outy) ? ifgreater_xy_in1_tmp : ifgreater_xy_in2_tmp;
    const float ifgreater_half_y_value2_tmp = 0.500000;
    vec3 ifgreater_half_y_out = (separate_uv_outy > ifgreater_half_y_value2_tmp) ? rotate_X1_out : rotate_X2_out;
    const float ifgreater_yx_in1_tmp = 1.000000;
    const float ifgreater_yx_in2_tmp = 0.000000;
    float ifgreater_yx_out = (separate_uv_outy > invert_x_out) ? ifgreater_yx_in1_tmp : ifgreater_yx_in2_tmp;
    vec3 select_plane_out = (ifgreater_xy_out == ifgreater_yx_out) ? ifgreater_half_x_out : ifgreater_half_y_out;
    vec3 rotate_normal_out = vec3(0.0);
    mx_rotate_vector3(select_plane_out, diag_select2_out, vec3(0.000000, 0.000000, 1.000000), rotate_normal_out);
    const float divide_half_in2_tmp = 2.000000;
    vec3 divide_half_out = rotate_normal_out / divide_half_in2_tmp;
