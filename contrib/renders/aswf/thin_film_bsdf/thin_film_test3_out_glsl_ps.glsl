...
uniform float diffuse_bsdf_weight = 0.500000;
uniform vec3 diffuse_bsdf_color = vec3(1.000000, 1.000000, 1.000000);
uniform float diffuse_bsdf_roughness = 0.000000;
uniform bool diffuse_bsdf_energy_compensation = false;
...
uniform vec2 dielectric_brdf_1_roughness = vec2(0.000000, 0.000000);
...
void main()
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            mx_oren_nayar_diffuse_bsdf(closureData, diffuse_bsdf_weight, diffuse_bsdf_color, diffuse_bsdf_roughness, geomprop_Nworld_out1, diffuse_bsdf_energy_compensation, diffuse_bsdf_out);
            BSDF layer_bsdf_2_out = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_bsdf(closureData, dielectric_brdf_1_out, diffuse_bsdf_out, layer_bsdf_2_out);

            surface1_out.color += occlusion * layer_bsdf_2_out.response;
        }
        ...
        BSDF layer_bsdf_2_out = BSDF(vec3(0.0),vec3(1.0));
        mx_layer_bsdf(closureData, dielectric_brdf_1_out, diffuse_bsdf_out, layer_bsdf_2_out);
        surface1_out.color += layer_bsdf_2_out.response;
