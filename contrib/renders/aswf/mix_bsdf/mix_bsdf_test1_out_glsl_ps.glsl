...
uniform float diffuse_brdf2_weight = 1.000000;
uniform vec3 diffuse_brdf2_color = vec3(0.000000, 0.730000, 0.100000);
uniform float diffuse_brdf2_roughness = 0.000000;
uniform bool diffuse_brdf2_energy_compensation = false;
...
void main()
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF diffuse_brdf2_out = BSDF(vec3(0.0),vec3(1.0));
            mx_oren_nayar_diffuse_bsdf(closureData, diffuse_brdf2_weight, diffuse_brdf2_color, diffuse_brdf2_roughness, geomprop_Nworld_out1, diffuse_brdf2_energy_compensation, diffuse_brdf2_out);
            BSDF mix_bsdf1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_mix_bsdf(closureData, diffuse_brdf1_out, diffuse_brdf2_out, mix_bsdf1_mix, mix_bsdf1_out);
