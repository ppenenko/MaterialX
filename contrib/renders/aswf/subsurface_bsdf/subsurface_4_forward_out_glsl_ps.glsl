...
uniform float subsurface_bsdf1_weight = 1.000000;
uniform vec3 subsurface_bsdf1_color = vec3(0.816000, 0.140000, 0.124000);
uniform vec3 subsurface_bsdf1_radius = vec3(1.000000, 1.000000, 1.000000);
uniform float subsurface_bsdf1_anisotropy = 0.900000;
...
void main()
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF subsurface_bsdf1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_subsurface_bsdf(closureData, subsurface_bsdf1_weight, subsurface_bsdf1_color, subsurface_bsdf1_radius, subsurface_bsdf1_anisotropy, geomprop_Nworld_out1, subsurface_bsdf1_out);

            surface1_out.color += occlusion * subsurface_bsdf1_out.response;
        }

        // Calculate the BSDF transmission for viewing direction
        BSDF subsurface_bsdf1_out = BSDF(vec3(0.0),vec3(1.0));
        surface1_out.color += subsurface_bsdf1_out.response;
