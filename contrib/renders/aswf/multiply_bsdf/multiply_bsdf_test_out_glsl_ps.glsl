...
uniform float diffuse_btdf1_weight = 1.000000;
uniform vec3 diffuse_btdf1_color = vec3(0.000000, 0.000000, 0.800000);
uniform float multiply_bsdf1_in2 = 0.500000;
uniform vec3 multiply_bsdf2_in2 = vec3(0.500000, 0.500000, 0.500000);
...
void main()
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF multiply_bsdf1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_multiply_bsdf_float(closureData, diffuse_brdf1_out, multiply_bsdf1_in2, multiply_bsdf1_out);
            BSDF diffuse_btdf1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_translucent_bsdf(closureData, diffuse_btdf1_weight, diffuse_btdf1_color, geomprop_Nworld_out1, diffuse_btdf1_out);
            BSDF multiply_bsdf2_out = BSDF(vec3(0.0),vec3(1.0));
            mx_multiply_bsdf_color3(closureData, diffuse_btdf1_out, multiply_bsdf2_in2, multiply_bsdf2_out);
            BSDF mix_bsdf1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_mix_bsdf(closureData, multiply_bsdf1_out, multiply_bsdf2_out, mix_bsdf1_mix, mix_bsdf1_out);
