...
uniform float clamp1_in = 0.500000;
uniform float clamp1_low = 0.000000;
uniform float clamp1_high = 1.000000;
uniform float invert1_amount = 1.000000;
uniform vec3 dielectric_brdf1_tint = vec3(1.000000, 1.000000, 1.000000);
uniform float dielectric_brdf1_ior = 1.520000;
...
uniform vec3 diffuse_brdf1_color = vec3(0.960000, 0.100000, 0.100000);
uniform float diffuse_brdf1_roughness = 0.000000;
uniform bool diffuse_brdf1_energy_compensation = false;
...
void main()
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            mx_dielectric_bsdf(closureData, clamp1_out, dielectric_brdf1_tint, dielectric_brdf1_ior, dielectric_brdf1_roughness, dielectric_brdf1_retroreflective, dielectric_brdf1_thinfilm_thickness, dielectric_brdf1_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, dielectric_brdf1_distribution, dielectric_brdf1_scatter_mode, dielectric_brdf1_out);
            BSDF diffuse_brdf1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_oren_nayar_diffuse_bsdf(closureData, invert1_out, diffuse_brdf1_color, diffuse_brdf1_roughness, geomprop_Nworld_out1, diffuse_brdf1_energy_compensation, diffuse_brdf1_out);
            BSDF add_bsdf1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_add_bsdf(closureData, dielectric_brdf1_out, diffuse_brdf1_out, add_bsdf1_out);

            surface1_out.color += occlusion * add_bsdf1_out.response;
        }
        ...
        BSDF add_bsdf1_out = BSDF(vec3(0.0),vec3(1.0));
        mx_add_bsdf(closureData, dielectric_brdf1_out, diffuse_brdf1_out, add_bsdf1_out);
        surface1_out.color += add_bsdf1_out.response;
