...
uniform float diffuse4_weight = 1.000000;
uniform vec3 diffuse4_color = vec3(0.600000, 0.600000, 0.600000);
uniform float diffuse4_roughness = 0.750000;
uniform bool diffuse4_energy_compensation = true;
uniform float surface4_opacity = 1.000000;
uniform bool surface4_thin_walled = false;
...
void main()
{
    ...
    surfaceshader surface4_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = surface4_opacity;
        ...
        // Add environment contribution
        {
            ...
            BSDF diffuse4_out = BSDF(vec3(0.0),vec3(1.0));
            mx_oren_nayar_diffuse_bsdf(closureData, diffuse4_weight, diffuse4_color, diffuse4_roughness, geomprop_Nworld_out1, diffuse4_energy_compensation, diffuse4_out);

            surface4_out.color += occlusion * diffuse4_out.response;
        }

        // Calculate the BSDF transmission for viewing direction
        BSDF diffuse4_out = BSDF(vec3(0.0),vec3(1.0));
        surface4_out.color += diffuse4_out.response;

        // Compute and apply surface opacity
        {
            surface4_out.color *= surfaceOpacity;
            surface4_out.transparency = mix(vec3(1.000000, 1.000000, 1.000000), surface4_out.transparency, surfaceOpacity);
        }
    }

    out41 = vec4(surface4_out.color, 1.0);
}
