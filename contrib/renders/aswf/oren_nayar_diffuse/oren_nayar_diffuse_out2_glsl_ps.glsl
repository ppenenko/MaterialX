...
uniform float diffuse2_weight = 1.000000;
uniform vec3 diffuse2_color = vec3(0.600000, 0.600000, 0.600000);
uniform float diffuse2_roughness = 0.250000;
uniform bool diffuse2_energy_compensation = true;
...
void main()
{
    ...
    surfaceshader surface2_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF diffuse2_out = BSDF(vec3(0.0),vec3(1.0));
            mx_oren_nayar_diffuse_bsdf(closureData, diffuse2_weight, diffuse2_color, diffuse2_roughness, geomprop_Nworld_out1, diffuse2_energy_compensation, diffuse2_out);

            surface2_out.color += occlusion * diffuse2_out.response;
        }

        // Calculate the BSDF transmission for viewing direction
        BSDF diffuse2_out = BSDF(vec3(0.0),vec3(1.0));
        surface2_out.color += diffuse2_out.response;
    ...
    out21 = vec4(surface2_out.color, 1.0);
}
