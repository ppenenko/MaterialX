...
uniform float zeltner_sheen_bsdf2_weight = 1.000000;
uniform vec3 zeltner_sheen_bsdf2_color = vec3(0.516000, 0.524000, 0.778000);
uniform float zeltner_sheen_bsdf2_roughness = 0.300000;
uniform int zeltner_sheen_bsdf2_mode = 1;
...
void main()
{
    ...
    surfaceshader surface2_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF zeltner_sheen_bsdf2_out = BSDF(vec3(0.0),vec3(1.0));
            mx_sheen_bsdf(closureData, zeltner_sheen_bsdf2_weight, zeltner_sheen_bsdf2_color, zeltner_sheen_bsdf2_roughness, geomprop_Nworld_out1, zeltner_sheen_bsdf2_mode, zeltner_sheen_bsdf2_out);

            surface2_out.color += occlusion * zeltner_sheen_bsdf2_out.response;
        }

        // Calculate the BSDF transmission for viewing direction
        BSDF zeltner_sheen_bsdf2_out = BSDF(vec3(0.0),vec3(1.0));
        surface2_out.color += zeltner_sheen_bsdf2_out.response;
    ...
    out11 = vec4(surface2_out.color, 1.0);
}
