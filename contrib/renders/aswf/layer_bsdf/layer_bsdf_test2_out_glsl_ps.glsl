...
uniform vec3 dielectric_interior_absorption = vec3(0.800000, 0.800000, 0.100000);
uniform vec3 dielectric_interior_scattering = vec3(0.000000, 0.000000, 0.000000);
uniform float dielectric_interior_anisotropy = 0.000000;
uniform float dielectric_R_weight = 1.000000;
uniform vec3 dielectric_R_tint = vec3(1.000000, 1.000000, 1.000000);
uniform float dielectric_R_ior = 1.800000;
uniform vec2 dielectric_R_roughness = vec2(0.050000, 0.050000);
uniform bool dielectric_R_retroreflective = false;
uniform float dielectric_R_thinfilm_thickness = 0.000000;
uniform float dielectric_R_thinfilm_ior = 1.500000;
uniform int dielectric_R_distribution = 0;
uniform int dielectric_R_scatter_mode = 0;
uniform float dielectric_T_weight = 1.000000;
uniform vec3 dielectric_T_tint = vec3(1.000000, 1.000000, 1.000000);
uniform float dielectric_T_ior = 1.500000;
uniform vec2 dielectric_T_roughness = vec2(0.050000, 0.050000);
uniform bool dielectric_T_retroreflective = false;
uniform float dielectric_T_thinfilm_thickness = 0.000000;
uniform float dielectric_T_thinfilm_ior = 1.500000;
uniform int dielectric_T_distribution = 0;
uniform int dielectric_T_scatter_mode = 1;
...
void main()
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF dielectric_R_out = BSDF(vec3(0.0),vec3(1.0));
            mx_dielectric_bsdf(closureData, dielectric_R_weight, dielectric_R_tint, dielectric_R_ior, dielectric_R_roughness, dielectric_R_retroreflective, dielectric_R_thinfilm_thickness, dielectric_R_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, dielectric_R_distribution, dielectric_R_scatter_mode, dielectric_R_out);
            BSDF dielectric_T_out = BSDF(vec3(0.0),vec3(1.0));
            mx_dielectric_bsdf(closureData, dielectric_T_weight, dielectric_T_tint, dielectric_T_ior, dielectric_T_roughness, dielectric_T_retroreflective, dielectric_T_thinfilm_thickness, dielectric_T_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, dielectric_T_distribution, dielectric_T_scatter_mode, dielectric_T_out);
            VDF dielectric_interior_out = VDF(vec3(0.0),vec3(1.0));
            mx_anisotropic_vdf(closureData, dielectric_interior_absorption, dielectric_interior_scattering, dielectric_interior_anisotropy, dielectric_interior_out);
            BSDF dielectric_T_with_interior_out = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_vdf(closureData, dielectric_T_out, dielectric_interior_out, dielectric_T_with_interior_out);
            BSDF dielectric_out = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_bsdf(closureData, dielectric_R_out, dielectric_T_with_interior_out, dielectric_out);

            surface1_out.color += occlusion * dielectric_out.response;
        }
        ...
        BSDF dielectric_out = BSDF(vec3(0.0),vec3(1.0));
        mx_layer_bsdf(closureData, dielectric_R_out, dielectric_T_with_interior_out, dielectric_out);
        surface1_out.color += dielectric_out.response;
