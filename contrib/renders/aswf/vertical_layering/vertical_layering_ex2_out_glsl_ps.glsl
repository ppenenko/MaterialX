...
uniform float oren_nayar_diffuse_bsdf1_weight = 1.000000;
uniform vec3 oren_nayar_diffuse_bsdf1_color = vec3(1.000000, 0.000000, 0.000000);
uniform float oren_nayar_diffuse_bsdf1_roughness = 0.000000;
uniform bool oren_nayar_diffuse_bsdf1_energy_compensation = false;
uniform float dielectric_bsdf3_weight = 1.000000;
uniform vec3 dielectric_bsdf3_tint = vec3(1.000000, 1.000000, 1.000000);
uniform float dielectric_bsdf3_ior = 3.000000;
uniform vec2 dielectric_bsdf3_roughness = vec2(0.050000, 0.050000);
uniform bool dielectric_bsdf3_retroreflective = false;
uniform float dielectric_bsdf3_thinfilm_thickness = 0.000000;
uniform float dielectric_bsdf3_thinfilm_ior = 1.500000;
uniform int dielectric_bsdf3_distribution = 0;
uniform int dielectric_bsdf3_scatter_mode = 0;
uniform float dielectric_bsdf1_weight = 1.000000;
uniform vec3 dielectric_bsdf1_tint = vec3(1.000000, 1.000000, 1.000000);
uniform float dielectric_bsdf1_ior = 1.500000;
uniform vec2 dielectric_bsdf1_roughness = vec2(0.050000, 0.050000);
uniform bool dielectric_bsdf1_retroreflective = false;
uniform float dielectric_bsdf1_thinfilm_thickness = 0.000000;
uniform float dielectric_bsdf1_thinfilm_ior = 1.500000;
uniform int dielectric_bsdf1_distribution = 0;
uniform int dielectric_bsdf1_scatter_mode = 0;
uniform float dielectric_bsdf2_weight = 1.000000;
uniform vec3 dielectric_bsdf2_tint = vec3(1.000000, 1.000000, 1.000000);
uniform float dielectric_bsdf2_ior = 3.000000;
uniform vec2 dielectric_bsdf2_roughness = vec2(0.050000, 0.050000);
uniform bool dielectric_bsdf2_retroreflective = false;
uniform float dielectric_bsdf2_thinfilm_thickness = 0.000000;
uniform float dielectric_bsdf2_thinfilm_ior = 1.500000;
uniform int dielectric_bsdf2_distribution = 0;
uniform int dielectric_bsdf2_scatter_mode = 0;
uniform float mix1_mix = 0.500000;
...
void main()
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF dielectric_bsdf3_out = BSDF(vec3(0.0),vec3(1.0));
            mx_dielectric_bsdf(closureData, dielectric_bsdf3_weight, dielectric_bsdf3_tint, dielectric_bsdf3_ior, dielectric_bsdf3_roughness, dielectric_bsdf3_retroreflective, dielectric_bsdf3_thinfilm_thickness, dielectric_bsdf3_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, dielectric_bsdf3_distribution, dielectric_bsdf3_scatter_mode, dielectric_bsdf3_out);
            BSDF dielectric_bsdf1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_dielectric_bsdf(closureData, dielectric_bsdf1_weight, dielectric_bsdf1_tint, dielectric_bsdf1_ior, dielectric_bsdf1_roughness, dielectric_bsdf1_retroreflective, dielectric_bsdf1_thinfilm_thickness, dielectric_bsdf1_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, dielectric_bsdf1_distribution, dielectric_bsdf1_scatter_mode, dielectric_bsdf1_out);
            BSDF oren_nayar_diffuse_bsdf1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_oren_nayar_diffuse_bsdf(closureData, oren_nayar_diffuse_bsdf1_weight, oren_nayar_diffuse_bsdf1_color, oren_nayar_diffuse_bsdf1_roughness, geomprop_Nworld_out1, oren_nayar_diffuse_bsdf1_energy_compensation, oren_nayar_diffuse_bsdf1_out);
            BSDF layer1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_bsdf(closureData, dielectric_bsdf1_out, oren_nayar_diffuse_bsdf1_out, layer1_out);
            BSDF dielectric_bsdf2_out = BSDF(vec3(0.0),vec3(1.0));
            mx_dielectric_bsdf(closureData, dielectric_bsdf2_weight, dielectric_bsdf2_tint, dielectric_bsdf2_ior, dielectric_bsdf2_roughness, dielectric_bsdf2_retroreflective, dielectric_bsdf2_thinfilm_thickness, dielectric_bsdf2_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, dielectric_bsdf2_distribution, dielectric_bsdf2_scatter_mode, dielectric_bsdf2_out);
            BSDF mix1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_mix_bsdf(closureData, layer1_out, dielectric_bsdf2_out, mix1_mix, mix1_out);
            BSDF layer2_out = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_bsdf(closureData, dielectric_bsdf3_out, mix1_out, layer2_out);

            surface1_out.color += occlusion * layer2_out.response;
        }
        ...
        BSDF layer2_out = BSDF(vec3(0.0),vec3(1.0));
        mx_layer_bsdf(closureData, dielectric_bsdf3_out, mix1_out, layer2_out);
        surface1_out.color += layer2_out.response;
