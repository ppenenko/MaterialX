...
uniform float translucent_bsdf_weight = 0.600000;
uniform vec3 translucent_bsdf_color = vec3(1.000000, 1.000000, 1.000000);
...
void main()
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            mx_translucent_bsdf(closureData, translucent_bsdf_weight, translucent_bsdf_color, geomprop_Nworld_out1, translucent_bsdf_out);

            surface1_out.color += occlusion * translucent_bsdf_out.response;
        }

        // Calculate the BSDF transmission for viewing direction
        BSDF translucent_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
        surface1_out.color += translucent_bsdf_out.response;
